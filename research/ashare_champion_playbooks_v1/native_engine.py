"""Minimal champion-native daily account derived from the audited V3 ledger."""
import json,math,time
from collections import defaultdict
import numpy as np
import pandas as pd
from pathlib import Path
from research.usic_multichampion_ashare_v3.common import CACHE,sha,dump,parquet,fee,quantity
from research.usic_multichampion_ashare_v3.legacy_replay import load_actions,ExecutionGap,metrics

HERE=Path(__file__).resolve().parent
OUT=Path('/Volumes/quant/CY_quant_research/ashare_champion_playbooks_v1/native')
...
class Market:
    def __init__(self):
        ...
        facts=HERE/'CORPORATE_ACTION_FACTS.csv'
        if facts.exists():
            for r in pd.read_csv(facts).to_dict('records'):
                mask=self.actions.event_id.eq(r['action_id']);assert int(mask.sum())==1,r['action_id']
                self.actions.loc[mask,'share_credit_date']=pd.Timestamp(r['share_credit_date'])
                self.actions.loc[mask,'pay_date']=pd.Timestamp(r['pay_date'])
        self.execution_binding=dict(actions=self.actions.attrs['input_bindings'],cache_manifest=sha(HERE.parent/'usic_multichampion_ashare_v3/INPUT_MANIFEST.json'))
...
def replay(m,s,frame):
    ...
    for t,day in enumerate(m.dates):
        ...
        for r,base in chosen:
            ...
            A=r['a0']*r['factor']/factor;S0=r['S0']*r['factor']/factor
            unitrisk=limit-S0+limit*.002
            if limit<=S0 or unitrisk<=0:orders.append(dict(base,status='INVALID_RISK_GEOMETRY'));continue
            cap=m.capacity[t-1,j]
            if not np.isfinite(cap):orders.append(dict(base,status='MISSING_PRIOR_CAPACITY'));continue
            budget_cap=min(priorcash,priornav*.15,cap+fee(cap,day,mult=s['cost']))
            maxq=quantity(budget_cap,limit,day,m.boards[j],s['cost'],maxq=cap/limit)
            maxq=quantity(budget_cap,limit,day,m.boards[j],s['cost'],maxq=min(maxq,priornav*.005/unitrisk,max(0.,priornav*.02-risk_existing-reserved_risk)/unitrisk))
        ...
        for ci,(r,base,j,limit,U,unitrisk,maxq,capcost,cap) in enumerate(prepared):
            maxq=min(maxq,int(max(0.,priornav*.02-risk_existing-reserved_risk)/unitrisk))
            q=quantity(float(budgets[ci]),limit,day,m.boards[j],s['cost'],maxq=maxq)
            if q<=0:
                lot=200 if m.boards[j]=='STAR' else 100;remaining=max(0.,priornav*.02-risk_existing-reserved_risk)
                status='AGGREGATE_RISK_REJECT' if remaining<lot*unitrisk else 'POSITION_RISK_LOT_REJECT' if priornav*.005<lot*unitrisk else 'CAPACITY_REJECT' if cap<lot*limit else 'CASH_OR_LOT_REJECT'
                orders.append(dict(base,status=status,budget=float(budgets[ci]),remaining_risk=remaining));continue
        ...
        for r,base,j,q,limit,U,reserve,unitrisk,isadd in commitments:
            ...
            p=dict(j=j,symbol=m.symbols[j],lot=nextlot,q=q,pending=0,e=t,signal_t=int(r['t']),entry=price,entry_reference=limit,entry_reference_coord=limit*entry_factor,entry_cash=debit,initial_q=q,mark=price,stale=0,due=t+60,planned=t+60,exit_reason='MAX60',S0coord=r['S0']*r['factor'],stop_coord=r['S0']*r['factor'],dividend_net=0.,dividend_accrued=0.,sell_value=0.,sell_fees=0.,lowopen=op<U,industry=int(r['industry']),planned_risk=q*unitrisk,initial_planned_r=q*unitrisk,initial_r_per_share=unitrisk,nav_at_order=priornav,buy_day_breach=False,intraday_only=0,winner=False,winner_t=-1,mfe_r=0.,mae_r=0.)
            p.update(family=r['route'],Acoord=r['a0']*r['factor'],risk_used=q*unitrisk,added=False,lots=[dict(e=t,q=q,price=price,role='INITIAL')],add_pnl=0.)
            if isadd:
            ...
            filled_risk_today+=q*unitrisk
            orders.append(dict(base,status='FILLED',limit=limit,opening=op,fill=price,quantity=q,reserved=reserve,debit=debit,lowopen=op<U,preauction_cash=priorcash,planned_risk=q*unitrisk,aggregate_risk_after=risk_existing+filled_risk_today,risk_cap=priornav*.02))
        for j,p in list(positions.items()):
            ...
            if p['pending']==0:
                ...
                pnl=p['sell_value']-p['sell_fees']+p['dividend_accrued']-p['entry_cash']
                trades.append(dict(p,exit_t=t,exit_date=day,entry_date=m.dates[p['e']],exit_price=price,held_sessions=t-p['e'],exit_delay=t-p['planned'],pnl=pnl,realized_r=pnl/p['initial_planned_r']))
                del positions[j]
        ...
        for a in m.records.get(day,[]):
            ...
            if missing:
                ...
                d=OUT/s['id'];d.mkdir(parents=True,exist_ok=True);pd.DataFrame(entitlements).to_csv(d/'action_gaps.csv',index=False)
        ...
        for p in positions.values():
            j=p['j'];factor=m.val('factor',t,j);cc=m.val('coord',t,j);lo=m.val('low',t,j)*factor;hi=m.val('high',t,j)*factor;s0=p['stop_coord']
            entry_coord=p['entry']*m.val('factor',p['e'],j);rcoord=p['initial_r_per_share']*m.val('factor',p['e'],j)
            p['mfe_r']=max(p['mfe_r'],(hi-entry_coord)/rcoord);p['mae_r']=min(p['mae_r'],(lo-entry_coord)/rcoord)
            if lo<=s0 and cc>s0:p['intraday_only']+=1
            if t==p['e'] and lo<=s0:p['buy_day_breach']=True
            reason=''
            if cc<=s0:reason='WINNER_TRAIL10' if p['winner'] and s0>p['S0coord'] else 'STRUCTURAL_INVALIDATION'
            elif t-p['e']>=60:reason='MAX60'
            elif not p['winner'] and (cc-entry_coord)>=2*rcoord:p['winner']=True;p['winner_t']=t
            elif p['winner'] and t>p['winner_t']:
                prior=m.a['close'][max(p['e'],t-10):t,j]*m.a['factor'][max(p['e'],t-10):t,j]
                if len(prior) and np.isfinite(prior).all():p['stop_coord']=max(p['stop_coord'],float(np.min(prior)))
                ref=p['entry_reference_coord']/factor
                p['risk_used']=0. if p['stop_coord']>=p['entry_reference_coord'] else p['q']*(ref-p['stop_coord']/factor+ref*.002)
                if cc<=p['stop_coord']:reason='WINNER_TRAIL10'
            if reason and p['due']>t+1:p['due']=t+1;p['planned']=t+1;p['exit_reason']=reason
        if False:
            for p in positions.values():
