#!/usr/bin/env python3
"""Freeze the outcome-blind weekly industry demand-shock mother identities."""
...
OUTPUT_ROOT = Path("/Volumes/quant/CY_quant_research/ashare_weekly_industry_demand_shock_direction_mother_v1")
CANDIDATES = OUTPUT_ROOT / "stage_a/candidates_frozen.parquet"
FREEZE = OUTPUT_ROOT / "stage_a/stage_a_freeze.json"
EXPECTED_SPEC = "a522af6a979572ace20c11e7fce9af05d3431c2ed54f901ea3e23b30c35f3dcb"
...
def build(con: duckdb.DuckDBPyConnection) -> None:
    ...
    WITH history AS (
      ...
      WHERE prior60_rows=60 AND lag60_cal_idx=cal_idx-60
        ...
        AND coord_open>0 AND coord_close>0 AND coord_high>=coord_low
      ...
      SELECT *,count(*) OVER(PARTITION BY trade_date) AS industry_count,
        max(industry_latest_available_at) OVER(PARTITION BY trade_date)
          AS market_latest_available_at
      FROM industry_daily
    ), ranked AS (
      SELECT *,
        row_number() OVER(PARTITION BY trade_date
          ORDER BY industry_score DESC,causal_industry) AS strongest_rank,
        row_number() OVER(PARTITION BY trade_date
          ORDER BY industry_score,causal_industry) AS weakest_rank
      FROM covered
      WHERE industry_count>=20 AND dayofweek(trade_date)=5
    ), lanes AS (
      SELECT 'CONTINUATION' AS direction_lane,* FROM ranked WHERE strongest_rank=1
      UNION ALL
      SELECT 'REVERSAL' AS direction_lane,* FROM ranked WHERE weakest_rank=1
    ), representatives AS (
      SELECT l.*,e.symbol,e.sleeve,e.decision_at,e.available_at,
        ...
        row_number() OVER(
          PARTITION BY l.trade_date,l.direction_lane
    ...
    SELECT direction_lane || '|' || strftime(trade_date,'%Y%m%d') || '|'
      ...
      industry_snapshot_id,industry_n,industry_score,industry_count,
      ...
      coord_close,step_return,amount,turnover_fraction,industry_median_distance
...
def summarize(con: duckdb.DuckDBPyConnection) -> dict[str, object]:
    audit_row = con.execute(
        f"""SELECT count(*),count(*)-count(DISTINCT event_id),
          ...
          count(*) FILTER(WHERE industry_count<20),
          count(*) FILTER(WHERE industry_median_distance IS NULL)
    ...
    audit = {
        ...
        "non_friday_signals": int(audit_row[4]),
        "coverage_failures": int(audit_row[5]),
        "medoid_failures": int(audit_row[6]),
    }
    if any(audit[key] for key in audit if key != "rows"):
        raise ResearchError(f"candidate audit failed: {audit}")
    dates = con.execute(
        f"""SELECT count(*) FROM (
          SELECT signal_date,count(*) AS n,count(DISTINCT direction_lane) AS lanes
          FROM read_parquet('{CANDIDATES.as_posix()}') GROUP BY signal_date
          HAVING n!=2 OR lanes!=2)"""
    ).fetchone()[0]
    if dates:
        raise ResearchError(f"{dates} signal dates do not have exactly two lanes")
    rows = con.execute(
        f"""SELECT year(signal_date),direction_lane,count(*),
          count(DISTINCT signal_date),count(DISTINCT causal_industry),
          count(DISTINCT symbol)
        FROM read_parquet('{CANDIDATES.as_posix()}') GROUP BY ALL ORDER BY 1,2"""
    ).fetchall()
    annual: dict[str, dict[str, dict[str, int]]] = {}
    for year,lane,n,n_dates,n_industries,n_symbols in rows:
        annual.setdefault(str(year),{})[str(lane)] = {
            "events": int(n),
            "dates": int(n_dates),
            "industries": int(n_industries),
