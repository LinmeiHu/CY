#!/usr/bin/env python3
"""Bull-market liquidation absorption with next-session confirmation."""
...
EXPERIMENT = "ASHARE-BULL-LIQUIDATION-ABSORPTION-CONFIRMATION-V11"
EXT = Path("/Volumes/quant/CY_quant_research/ashare_bull_liquidation_absorption_confirmation_v11")
...
def contract_value() -> dict[str, Any]:
    return {
        ...
        "economic_hypothesis": (
            "Inside a causal broad BULL state and healthy industry, a former strong "
            "stock's lower-low positive reversal is only provisional absorption. The "
            "next completed session must close above the reversal high and in its own "
            "upper 30% before supply transfer is treated as confirmed."
        ),
        "market": v1.contract_value()["market_regime"],
        "industry": "confirmation-day median ret20>0 and positive-ret20 share>50%",
        "reversal_day": {
            "former_strength": "ret60 known before reversal>=10% or 20%",
            "liquidation": "new five-session low >=10% or 15% below prior20 high",
            "provisional_absorption": (
                "close>=open; close location>=70%; turnover>=preceding20 mean"
            ),
        },
        "confirmation_day": ("next market session; close>=reversal high and close location>=70%"),
        "cooldown": "first confirmation per symbol in each causal 10-session window",
        "rules": {
            "R1_STRENGTH10_DD10": "strength>=10%; drawdown>=10%",
            "R2_STRENGTH20_DD10": "strength>=20%; drawdown>=10%",
            "R3_STRENGTH10_DD15": "strength>=10%; drawdown>=15%",
            "R4_STRENGTH20_DD15": "strength>=20%; drawdown>=15%",
        },
        "profiles": PROFILES,
        "entry": "first legal next daily open after completed confirmation",
...
def persist_contract() -> dict[str, str]:
    ...
    v1.write_json(
        ...
        {
            "experiment": EXPERIMENT,
            "status": "OUTCOME_BLIND_ABSORPTION_CONFIRMATION_FREEZE",
...
def build_candidates() -> pd.DataFrame:
    ...
    try:
        con.execute(
            ...
            COPY (
              WITH d0 AS (
                SELECT *,lag(ret60,2) OVER w AS pre_reversal_ret60,
                  max(coord_high) OVER w20 AS pre_reversal20_high,
                  min(coord_low) OVER w5 AS pre_reversal5_low,
                  avg(turnover_fraction) OVER w20 AS pre_reversal20_turnover,
                  count(*) OVER w20 AS history_n20,
                  lag(coord_low) OVER w AS reversal_low,
                  lag(coord_high) OVER w AS reversal_high,
                  lag(coord_close) OVER w AS reversal_close,
                  lag(coord_open) OVER w AS reversal_open,
                  lag(turnover_fraction) OVER w AS reversal_turnover,
                  lag(cal_idx) OVER w AS reversal_cal_idx
                ...
                WINDOW w AS (PARTITION BY symbol ORDER BY trade_date),
                  ...
                  w5 AS (PARTITION BY symbol ORDER BY trade_date
                    ROWS BETWEEN 6 PRECEDING AND 2 PRECEDING)
              ), x AS (
                SELECT d0.*,r.market_regime,r.market_median_ret20,
                  ...
                  reversal_low/pre_reversal20_high-1 AS liquidation_drawdown,
                  (coord_close-coord_low)/nullif(coord_high-coord_low,0)
                    AS confirmation_close_location,
                  reversal_turnover/nullif(pre_reversal20_turnover,0)
                ...
                WHERE trade_date>=DATE '2014-01-01' AND history_n20=20
                  AND cal_idx-reversal_cal_idx=1 AND r.market_regime='BULL'
                  AND i.industry_n20>=5 AND i.industry_median_ret20>0
                  AND i.industry_positive_ret20_share>0.50
                  AND pre_reversal_ret60>=0.10
                  AND reversal_low/pre_reversal20_high-1<=-0.10
                  AND reversal_low<=pre_reversal5_low
                  AND reversal_close>=reversal_open
                  AND (reversal_close-reversal_low)
                    /nullif(reversal_high-reversal_low,0)>=0.70
                  AND reversal_turnover>=pre_reversal20_turnover
                  AND coord_close>=reversal_high
                  AND (coord_close-coord_low)/nullif(coord_high-coord_low,0)>=0.70
              )
              SELECT *,'BAC-'||strftime(trade_date,'%Y%m%d')||'-'||symbol AS event_id,
                trade_date AS signal_date,decision_at AS feature_latest_timestamp,
                pre_reversal_ret60 AS prior_runup,
                liquidation_drawdown AS pre_drawdown,
                pre_reversal20_high AS prior20_high,
                pre_reversal20_high AS platform_high,
                TRUE AS pass_R1_STRENGTH10_DD10,
                pre_reversal_ret60>=0.20 AS pass_R2_STRENGTH20_DD10,
                liquidation_drawdown<=-0.15 AS pass_R3_STRENGTH10_DD15,
                pre_reversal_ret60>=0.20 AND liquidation_drawdown<=-0.15
                  AS pass_R4_STRENGTH20_DD15
...
def sample_and_render(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    sample.insert(0, "chart_id", [f"BULL-CONFIRM-{i:03d}" for i in range(1, 41)])
...
def run_stage_a() -> dict[str, Any]:
    ...
    cooldown_violations = sum(
        int(part.cal_idx.sort_values().diff().le(10).sum()) for _, part in frame.groupby("symbol")
    )
    audit = {
    ...
    freeze = {
        ...
        **hashes,
        ...
        "v11_runner_sha256": v1.sha256(Path(__file__)),
...
def run_stage_b() -> dict[str, Any]:
    ...
    result["verdict"] = (
        "BULL_LIQUIDATION_ABSORPTION_CONFIRMATION_EDGE"
        if passed
        else "BULL_LIQUIDATION_ABSORPTION_CONFIRMATION_FAILS_TARGET"
    )
