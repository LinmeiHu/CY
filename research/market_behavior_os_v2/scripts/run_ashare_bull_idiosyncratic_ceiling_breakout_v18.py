#!/usr/bin/env python3
"""Bull-market idiosyncratic 120-day ceiling-breakout research."""
...
EXPERIMENT = "ASHARE-BULL-IDIOSYNCRATIC-CEILING-BREAKOUT-V18"
EXT = Path("/Volumes/quant/CY_quant_research/ashare_bull_idiosyncratic_ceiling_breakout_v18")
...
RULES = {
    "R1_NONCROWDED_INDUSTRY": {},
    "R2_LOW_CROWDING_INDUSTRY": {},
    "R3_NONCROWDED_BOUNDED_HISTORY": {},
    "R4_NONCROWDED_QUIET_TURNOVER": {},
}
...
def contract_value() -> dict[str, Any]:
    return {
        ...
        "economic_hypothesis": (
            "In a causal broad BULL market, a first 120-session stock breakout inside "
            "an absolutely healthy but cross-sectionally non-crowded industry represents "
            "stock-specific information discovery before sector-wide consensus. This "
            "avoids paying for already-crowded industry momentum."
        ),
        "market": v1.contract_value()["market_regime"],
        "industry": {
            "absolute_health": "median ret20>0 and positive-ret20 share>50%",
            "noncrowded": "same-day PIT industry ret20 percentile<=50%",
            "low_crowding": "same-day PIT industry ret20 percentile<=30%",
        },
        "stock": {
            "history": "120 prior valid completed sessions",
            "first_breakout": "close>=prior120 high; previous close<its prior120 high",
            "acceptance": "close>=open and close location>=70%",
            "demand": "turnover>=1.5x preceding20 mean",
            "extension": "pre-signal ret20<=15%",
            "cooldown": "first retained signal per symbol in causal 30-session window",
        },
        "rules": {
            "R1_NONCROWDED_INDUSTRY": "industry percentile<=50%",
            "R2_LOW_CROWDING_INDUSTRY": "industry percentile<=30%",
            "R3_NONCROWDED_BOUNDED_HISTORY": "R1 and prior120 range<=60%",
            "R4_NONCROWDED_QUIET_TURNOVER": "R1 and prior20 mean turnover<=prior60 mean",
        },
        "profiles": PROFILES,
        ...
        "governance": {
            "independent_from_gap_repair": True, "frozen_before_outcomes": True,
...
def persist_contract() -> dict[str, str]:
    ...
    v1.write_json(SPEC, {
        "experiment": EXPERIMENT,
        "status": "OUTCOME_BLIND_IDIOSYNCRATIC_BREAKOUT_FREEZE",
            ...
            COPY (
              WITH d0 AS (
                ...
                WHERE trade_date>=DATE '2014-01-01' AND history_n120=120
                  ...
                  AND i.industry_ret20_percentile<=0.50
                  AND coord_close>=prior120_high AND previous_close<previous_prior120_high
                  AND signal_close_location>=0.70 AND coord_close>=coord_open
                  AND turnover_fraction>=1.50*prior20_turnover AND prior_ret20<=0.15
              )
              SELECT *,'BICB-'||strftime(trade_date,'%Y%m%d')||'-'||symbol AS event_id,
                ...
                TRUE AS pass_R1_NONCROWDED_INDUSTRY,
                industry_ret20_percentile<=0.30 AS pass_R2_LOW_CROWDING_INDUSTRY,
                prior120_range<=0.60 AS pass_R3_NONCROWDED_BOUNDED_HISTORY,
                quiet_turnover_ratio<=1 AS pass_R4_NONCROWDED_QUIET_TURNOVER
...
def sample_and_render(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    sample.insert(0, "chart_id", [f"BULL-IDIO-{i:03d}" for i in range(1, 41)])
...
def run_stage_a() -> dict[str, Any]:
    ...
    freeze = {"experiment": EXPERIMENT, **hashes,
        ...
        "v18_runner_sha256": v1.sha256(Path(__file__)),
...
def run_stage_b() -> dict[str, Any]:
    ...
    try:
        for name, value in values.items(): setattr(v6, name, value)
        result = v6.run_stage_b()
    finally:
        for name, value in old.items(): setattr(v6, name, value)
    gate = result.get("gate"); passed = isinstance(gate, dict) and bool(gate) and all(gate.values())
    result["experiment"] = EXPERIMENT
    result["verdict"] = "BULL_IDIOSYNCRATIC_CEILING_BREAKOUT_EDGE" if passed else "BULL_IDIOSYNCRATIC_CEILING_BREAKOUT_FAILS_TARGET"
