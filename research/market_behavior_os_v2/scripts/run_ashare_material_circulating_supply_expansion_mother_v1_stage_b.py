#!/usr/bin/env python3
"""Attach fixed executable anatomy outcomes to material supply expansions."""
...
def replay_one(
    ...
    common = {
        ...
        "previous_circulating_shares": float(candidate.previous_circulating_shares),
        "circulating_shares": float(candidate.circulating_shares),
        ...
        result.update(
            {
                ...
                f"{prefix}_mfe": float(held.coord_high.max() / entry_price - 1.0) if not held.empty else math.nan,
                f"{prefix}_mae": float(held.coord_low.min() / entry_price - 1.0) if not held.empty else math.nan,
...
def run() -> dict[str, Any]:
    ...
    if candidates.signal_date.dt.year.gt(2020).any():
        raise ResearchError("post-2020 signal identity entered Stage B")
    paths = build_paths()
    path_lookup = {key: part for key, part in paths.groupby("event_id", sort=False)}
    ind = industry_lookup()
    outcomes = pd.DataFrame(
        [
            replay_one(candidate, path_lookup[str(candidate.event_id)], ind)
    ...
    manifest = {
        ...
        "completed_h120": int(outcomes.h120_status.eq("COMPLETED").sum()),
        "status_counts": {
            str(key): int(value) for key, value in outcomes.status.value_counts().sort_index().items()
        },
        "annual_or_pooled_aggregate_produced": False,
