#!/usr/bin/env python3
"""Bull-market quiet-base demand-shock research."""
...
EXPERIMENT = "ASHARE-BULL-QUIET-BASE-DEMAND-SHOCK-V20"
EXT = Path("/Volumes/quant/CY_quant_research/ashare_bull_quiet_base_demand_shock_v20")
...
RULES = {
    "R1_QUIET_BASE_NINE_PCT": {},
    "R2_TIGHT_BASE_NINE_PCT": {},
    "R3_EARLY_SEVEN_PCT": {},
    "R4_HIGH_DEMAND_SEVEN_PCT": {},
}
...
def contract_value() -> dict[str, Any]:
    return {
        ...
        "economic_hypothesis": (
            "In a causally observable broad BULL market and healthy PIT industry, "
            "a stock that has remained inside a bounded sixty-session inventory base "
            "and then closes near the high on an exceptional positive return and "
            "turnover shock is entering an early demand-led markup phase."
        ),
        "market": v1.contract_value()["market_regime"],
        "industry": "PIT industry median ret20>0 and positive-ret20 share>50%",
        "stock": {
            "base": "prior 60 valid sessions only",
            "demand_shock": "completed signal daily return/turnover/close location",
            "cooldown": "first retained signal per symbol in causal 20-session window",
        },
        "rules": {
            "R1_QUIET_BASE_NINE_PCT": "range60<=35%, prior ret20<=10%, return>=9%, turnover>=1.5x, location>=90%",
            "R2_TIGHT_BASE_NINE_PCT": "R1 with range60<=25%",
            "R3_EARLY_SEVEN_PCT": "range60<=35%, prior ret20<=5%, return>=7%, turnover>=1.5x, location>=85%",
            "R4_HIGH_DEMAND_SEVEN_PCT": "range60<=30%, prior ret20<=10%, return>=7%, turnover>=2x, location>=90%",
        },
        "profiles": PROFILES,
...
def persist_contract() -> dict[str, str]:
    ...
    v1.write_json(SPEC, {"experiment": EXPERIMENT,
        "status": "OUTCOME_BLIND_QUIET_BASE_DEMAND_SHOCK_FREEZE",
    ...
    for _, part in frame.sort_values(["symbol", "cal_idx"]).groupby("symbol", sort=False):
        ...
        for idx, row in part.iterrows():
            if int(row.cal_idx) > last + 20:
                kept.append(idx); last = int(row.cal_idx)
        ...
        con.execute(f"""
          COPY (
            WITH d0 AS (
              ...
              SELECT d0.*,r.market_regime,r.market_median_ret20,r.market_positive_ret20_share,
              ...
              WHERE trade_date>=DATE '2014-01-01' AND history_n60=60
                AND r.market_regime='BULL' AND i.industry_n20>=5
                AND i.industry_median_ret20>0 AND i.industry_positive_ret20_share>0.50
            ), f AS (
              SELECT *,
                prior60_range<=0.35 AND prior_ret20<=0.10 AND step_return>=0.09
                  AND turnover_expansion>=1.50 AND signal_close_location>=0.90 AS pass_R1_QUIET_BASE_NINE_PCT,
                prior60_range<=0.25 AND prior_ret20<=0.10 AND step_return>=0.09
                  AND turnover_expansion>=1.50 AND signal_close_location>=0.90 AS pass_R2_TIGHT_BASE_NINE_PCT,
                prior60_range<=0.35 AND prior_ret20<=0.05 AND step_return>=0.07
                  AND turnover_expansion>=1.50 AND signal_close_location>=0.85 AS pass_R3_EARLY_SEVEN_PCT,
                prior60_range<=0.30 AND prior_ret20<=0.10 AND step_return>=0.07
                  AND turnover_expansion>=2.00 AND signal_close_location>=0.90 AS pass_R4_HIGH_DEMAND_SEVEN_PCT
              FROM x
            )
            SELECT *,'BQDS-'||strftime(trade_date,'%Y%m%d')||'-'||symbol AS event_id,
              trade_date AS signal_date,decision_at AS feature_latest_timestamp,
              prior_ret20 AS prior_runup,coord_close/prior60_high-1 AS pre_drawdown,
              prior20_high,prior60_high AS platform_high
            FROM f WHERE pass_R1_QUIET_BASE_NINE_PCT OR pass_R2_TIGHT_BASE_NINE_PCT
              OR pass_R3_EARLY_SEVEN_PCT OR pass_R4_HIGH_DEMAND_SEVEN_PCT
            ORDER BY trade_date,symbol
          ) TO '{RAW_CANDIDATES}' (FORMAT PARQUET,COMPRESSION ZSTD)
        """)
    finally:
        con.close()
    raw = v1.read_parquet_duckdb(RAW_CANDIDATES); frame = _cooldown(raw); v1.write_parquet(frame, CANDIDATES)
...
def sample_and_render(frame: pd.DataFrame) -> pd.DataFrame:
    work = frame.copy(); work["hash_order"] = work.event_id.map(lambda x: hashlib.sha256(str(x).encode()).hexdigest())
    work["year"] = work.signal_date.dt.year
    pieces = [part.sort_values("hash_order").head(2) for _, part in work.groupby(["year", "sleeve"], sort=True)]
    ...
    sample.insert(0, "chart_id", [f"BULL-DEMAND-{i:03d}" for i in range(1,41)])
...
def run_stage_a() -> dict[str, Any]:
    ...
    coverage={rule:{"count":int(frame[f"pass_{rule}"].sum()),"annual":(frame.loc[frame[f"pass_{rule}"]]
        .groupby(frame.loc[frame[f"pass_{rule}"],"signal_date"].dt.year).size().reindex(YEARS,fill_value=0).astype(int).to_dict())}
        for rule in RULES}
    audit={"duplicate_event_count":int(frame.event_id.duplicated().sum()),
    ...
    freeze={"experiment":EXPERIMENT,**hashes,"runner_sha256":v1.sha256(Path(v6.__file__)),
            "v20_runner_sha256":v1.sha256(Path(__file__)),"raw_candidate_sha256":v1.sha256(RAW_CANDIDATES),
            "candidate_sha256":v1.sha256(CANDIDATES),"blind_pdf_sha256":v1.sha256(BLIND_PDF),
            "candidate_count":len(frame),"blind_chart_count":len(sample),"coverage":coverage,
            "audit":audit,"outcomes_opened":False}
    v1.write_json(FREEZE,freeze); return freeze


def verify_own_freeze() -> None:
    freeze=json.loads(FREEZE.read_text()); expected={"contract_sha256":v1.sha256(CONTRACT),
        "spec_sha256":v1.sha256(SPEC),"v20_runner_sha256":v1.sha256(Path(__file__)),
        "raw_candidate_sha256":v1.sha256(RAW_CANDIDATES),"candidate_sha256":v1.sha256(CANDIDATES),
...
def run_stage_b() -> dict[str, Any]:
    verify_own_freeze()
    names=("EXPERIMENT","CONTRACT","SPEC","FREEZE","RESULT","REPORT","CANDIDATES","BLIND_PDF",
           "OUTCOMES","ACCEPTED","SKIPPED","NAV","RULES","PROFILES")
    values={n:globals()[n] for n in names}; old={n:getattr(v6,n) for n in names}
    try:
    ...
    result["verdict"]="BULL_QUIET_BASE_DEMAND_SHOCK_EDGE" if passed else "BULL_QUIET_BASE_DEMAND_SHOCK_FAILS_TARGET"
