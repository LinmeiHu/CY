#!/usr/bin/env python3
"""Bull-market former-leader liquidation absorption research."""
...
EXPERIMENT = "ASHARE-BULL-FORMER-LEADER-LIQUIDATION-ABSORPTION-V9"
EXT = Path("/Volumes/quant/CY_quant_research/ashare_bull_former_leader_liquidation_absorption_v9")
...
def contract_value() -> dict[str, Any]:
    return {
        ...
        "economic_hypothesis": (
            "When the broad market and the stock's PIT industry remain healthy, a "
            "previously strong stock can suffer an idiosyncratic short liquidation. "
            "A new five-session low that reverses to a positive body in the upper 30% "
            "of its range on above-normal turnover is observable full absorption of "
            "that temporary supply and may mean-revert quickly."
        ),
        "market": v1.contract_value()["market_regime"],
        "industry": "median ret20>0 and positive-ret20 share>50%",
        "event": {
            "former_strength": "prior completed-session ret60>=10% or 20%",
            "liquidation": (
                "signal low is new prior-five-session low and >=10% or 15% below prior20 high"
            ),
            "absorption": ("close>=open; close location>=70%; turnover>=prior20 mean"),
...
def persist_contract() -> dict[str, str]:
    ...
    v1.write_json(
        ...
        {
            "experiment": EXPERIMENT,
            "status": "OUTCOME_BLIND_LIQUIDATION_ABSORPTION_FREEZE",
            ...
            COPY (
              WITH d0 AS (
                ...
                SELECT d0.*,r.market_regime,r.market_median_ret20,
                  ...
                  coord_low/prior20_high-1 AS liquidation_drawdown,
                ...
                WHERE trade_date>=DATE '2014-01-01' AND history_n20=20
                  ...
                  AND prior_ret60>=0.10 AND coord_low/prior20_high-1<=-0.10
                  AND coord_low<=prior5_low AND coord_close>=coord_open
                  AND (coord_close-coord_low)/nullif(coord_high-coord_low,0)>=0.70
                  AND turnover_fraction>=prior20_turnover
              )
              SELECT *,'BABS-'||strftime(trade_date,'%Y%m%d')||'-'||symbol AS event_id,
                trade_date AS signal_date,decision_at AS feature_latest_timestamp,
                prior_ret60 AS prior_runup,liquidation_drawdown AS pre_drawdown,
                prior20_high,prior20_high AS platform_high,
                TRUE AS pass_R1_STRENGTH10_DD10,
                prior_ret60>=0.20 AS pass_R2_STRENGTH20_DD10,
                liquidation_drawdown<=-0.15 AS pass_R3_STRENGTH10_DD15,
                prior_ret60>=0.20 AND liquidation_drawdown<=-0.15
...
def sample_and_render(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    sample.insert(0, "chart_id", [f"BULL-ABSORB-{i:03d}" for i in range(1, 41)])
...
def run_stage_a() -> dict[str, Any]:
    ...
    for _, part in frame.groupby("symbol"):
        cooldown_violations += int(part.cal_idx.sort_values().diff().le(10).sum())
    audit = {
    ...
    freeze = {
        ...
        **hashes,
        ...
        "v9_runner_sha256": v1.sha256(Path(__file__)),
...
def run_stage_b() -> dict[str, Any]:
    ...
    result["verdict"] = (
        "BULL_FORMER_LEADER_LIQUIDATION_ABSORPTION_EDGE"
        if passed
        else "BULL_FORMER_LEADER_LIQUIDATION_ABSORPTION_FAILS_TARGET"
    )
