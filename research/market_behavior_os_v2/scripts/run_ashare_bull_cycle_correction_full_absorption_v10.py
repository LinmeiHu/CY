#!/usr/bin/env python3
"""Long-horizon bull-cycle correction full-absorption research."""
...
EXPERIMENT = "ASHARE-BULL-CYCLE-CORRECTION-FULL-ABSORPTION-V10"
EXT = Path("/Volumes/quant/CY_quant_research/ashare_bull_cycle_correction_full_absorption_v10")
...
RULES = {
    "R1_BULL_CORRECTION": {},
    "R2_INDUSTRY_LONG_HEALTHY": {},
    "R3_DEEP_STOCK_CORRECTION": {},
    "R4_GAP_SHOCK_ABSORBED": {},
}
...
def contract_value() -> dict[str, Any]:
    return {
        ...
        "economic_hypothesis": (
            "A causal long-horizon bull cycle can contain a short-horizon market "
            "correction. During that correction, a stock already down at least 10% "
            "over twenty sessions that makes a new five-session low but closes above "
            "the prior high in the upper 30% of its range on normal-or-higher turnover "
            "shows completed-session supply absorption and may rebound rapidly."
        ),
        "market_state": {
            "BULL_CYCLE": "market median ret60>0 and positive-ret60 share>50%",
            "BEAR_CYCLE": "market median ret60<=0 and positive-ret60 share<=50%",
            "TRANSITION_CYCLE": "remaining mixed states",
            "correction": "median ret20<=0 or positive-ret20 share<=50%",
        },
        "event": {
            "stock_correction": "prior completed-session ret20<=-10%",
            "new_low": "signal low<=prior-five-session low",
            "full_absorption": (
                "close>=prior-session high; close location>=70%; turnover>=prior20 mean"
            ),
            "cooldown": "retain first event per symbol in each causal 10-session window",
        },
        "rules": {
            "R1_BULL_CORRECTION": "base contract",
            "R2_INDUSTRY_LONG_HEALTHY": "R1 and industry median ret60>0",
            "R3_DEEP_STOCK_CORRECTION": "R1 and prior ret20<=-15%",
            "R4_GAP_SHOCK_ABSORBED": "R1 and signal open<=99% of prior close",
        },
        "profiles": PROFILES,
...
def persist_contract() -> dict[str, str]:
    ...
    v1.write_json(
        ...
        {
            "experiment": EXPERIMENT,
            "status": "OUTCOME_BLIND_BULL_CYCLE_CORRECTION_FREEZE",
        ...
        con.execute(
            ...
            COPY (
              WITH d0 AS (
                ...
                SELECT d0.*,r.market_median_ret20,r.market_positive_ret20_share,
                  ...
                  i.industry_positive_ret20_share,i.industry_ret20_percentile,i.industry_n20,
                  ...
                  coord_open/nullif(prior_coord_close,0)-1 AS open_gap,
                  ret20-i.industry_median_ret20 AS stock_minus_industry_ret20,
                  'BULL' AS market_regime
                ...
                WHERE trade_date>=DATE '2014-01-01' AND history_n20=20
                  ...
                  AND (r.market_median_ret20<=0 OR r.market_positive_ret20_share<=0.50)
                  AND prior_ret20<=-0.10 AND coord_low<=prior5_low
                  AND coord_close>=prior_high
                  AND (coord_close-coord_low)/nullif(coord_high-coord_low,0)>=0.70
                  AND turnover_fraction>=prior20_turnover
              )
              SELECT *,'BCFA-'||strftime(trade_date,'%Y%m%d')||'-'||symbol AS event_id,
                trade_date AS signal_date,decision_at AS feature_latest_timestamp,
                prior_ret20 AS prior_runup,prior_ret20 AS pre_drawdown,
                prior_high AS prior20_high,prior_high AS platform_high,
                TRUE AS pass_R1_BULL_CORRECTION,
                industry_median_ret60>0 AS pass_R2_INDUSTRY_LONG_HEALTHY,
                prior_ret20<=-0.15 AS pass_R3_DEEP_STOCK_CORRECTION,
                open_gap<=-0.01 AS pass_R4_GAP_SHOCK_ABSORBED
...
def sample_and_render(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    sample.insert(0, "chart_id", [f"BULL-CYCLE-{i:03d}" for i in range(1, 41)])
...
def run_stage_a() -> dict[str, Any]:
    ...
    freeze = {
        ...
        **hashes,
        ...
        "v10_runner_sha256": v1.sha256(Path(__file__)),
...
def run_stage_b() -> dict[str, Any]:
    ...
    result["verdict"] = (
        "BULL_CYCLE_CORRECTION_FULL_ABSORPTION_EDGE"
        if passed
        else "BULL_CYCLE_CORRECTION_FULL_ABSORPTION_FAILS_TARGET"
    )
