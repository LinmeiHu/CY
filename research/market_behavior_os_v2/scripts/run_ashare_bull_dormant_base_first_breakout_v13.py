#!/usr/bin/env python3
"""Bull-market dormant-base first-breakout research."""
...
EXPERIMENT = "ASHARE-BULL-DORMANT-BASE-FIRST-BREAKOUT-V13"
EXT = Path("/Volumes/quant/CY_quant_research/ashare_bull_dormant_base_first_breakout_v13")
...
RULES = {
    "R1_DORMANT_BASE_BREAKOUT": {},
    "R2_DEEP_QUIET_BASE": {},
    "R3_HIGH_DEMAND_BREAKOUT": {},
    "R4_TIGHT_BASE": {},
}
...
def contract_value() -> dict[str, Any]:
    return {
        ...
        "economic_hypothesis": (
            "During a causal broad BULL state and healthy industry, a stock that has "
            "spent sixty sessions in a bounded base while its recent turnover is no "
            "higher than its sixty-session norm has reduced immediate supply. Its first "
            "close above the completed sixty-session ceiling on expanded turnover is a "
            "new price-discovery event."
        ),
        ...
        "base": {
            "prior60_range": "<=40%",
            "prior20_range": "<=20%",
            "quiet_turnover": "preceding20 mean turnover<=preceding60 mean turnover",
            "extension": "ret20 known before signal<=15%",
        },
        "trigger": {
            "first_breakout": (
                "close>=preceding60 high and previous close<its then-preceding60 high"
            ),
            "bar_acceptance": "close>=open and close location>=70%",
            "demand": "turnover>=1.5x preceding20 mean",
            "cooldown": "first retained signal per symbol in each causal 20-session window",
        },
        "rules": {
            "R1_DORMANT_BASE_BREAKOUT": "base contract",
            "R2_DEEP_QUIET_BASE": "R1 and preceding20/preceding60 turnover<=0.80",
            "R3_HIGH_DEMAND_BREAKOUT": "R1 and turnover expansion>=2.0",
            "R4_TIGHT_BASE": "R1 and prior60 range<=35% and prior20 range<=15%",
        },
        "profiles": PROFILES,
...
def persist_contract() -> dict[str, str]:
    ...
    v1.write_json(
        ...
        {
            "experiment": EXPERIMENT,
            "status": "OUTCOME_BLIND_DORMANT_BASE_BREAKOUT_FREEZE",
            ...
            COPY (
              WITH d0 AS (
                ...
                SELECT *,lag(coord_close) OVER w AS previous_close,
                  lag(prior60_high) OVER w AS previous_prior60_high,
                ...
                WHERE trade_date>=DATE '2014-01-01' AND history_n60=60
                  ...
                  AND previous_close<previous_prior60_high
              ...
              SELECT *,'BDB-'||strftime(trade_date,'%Y%m%d')||'-'||symbol AS event_id,
                trade_date AS signal_date,decision_at AS feature_latest_timestamp,
                prior_ret60 AS prior_runup,coord_close/prior60_high-1 AS pre_drawdown,
                prior60_high AS prior20_high,prior60_high AS platform_high,
                TRUE AS pass_R1_DORMANT_BASE_BREAKOUT,
                quiet_turnover_ratio<=0.80 AS pass_R2_DEEP_QUIET_BASE,
                turnover_expansion>=2.00 AS pass_R3_HIGH_DEMAND_BREAKOUT,
                prior60_range<=0.35 AND prior20_range<=0.15 AS pass_R4_TIGHT_BASE
...
def sample_and_render(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    sample.insert(0, "chart_id", [f"BULL-BASE-{i:03d}" for i in range(1, 41)])
...
def run_stage_a() -> dict[str, Any]:
    ...
    freeze = {
        ...
        **hashes,
        ...
        "v13_runner_sha256": v1.sha256(Path(__file__)),
...
def run_stage_b() -> dict[str, Any]:
    ...
    result["verdict"] = (
        "BULL_DORMANT_BASE_FIRST_BREAKOUT_EDGE"
        if passed
        else "BULL_DORMANT_BASE_FIRST_BREAKOUT_FAILS_TARGET"
    )
