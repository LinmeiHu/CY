#!/usr/bin/env python3
"""Render every frozen bull-deceleration pullback-absorption event."""
...
STAGE_A_RUNNER = REPO / (
    ...
    "run_ashare_bull_deceleration_controlled_pullback_absorption_mother_v1_stage_a.py"
)
STAGE_B_RUNNER = REPO / (
    "research/market_behavior_os_v2/scripts/"
    "run_ashare_bull_deceleration_controlled_pullback_absorption_mother_v1_stage_b.py"
)
...
EXPECTED_HASHES = {
    ...
    STAGE_A_RUNNER: "6ed6a04f0e2e1966639224511841b73d23c5f2be86422faefc4344d65c0f9dc7",
    STAGE_B_RUNNER: "89132e453e388d37d394c6e0a4b11d5647595ad58b4be48352195e7380ddf596",
    DAILY: "53c9e1e62b4b1bbd45979c868f5543eea58114d0bf659f866cd37d979068cc00",
    STAGE_A_FREEZE: "207323753ee9d42aea1cf22ac0a6dd90d34f2871d98048d114cee0a8d942358b",
    ...
    OUTCOMES: "1874f9a25123fe903be8598ef89d307ec47b236d426df5bda415dff3b3229664",
}
...
def load_inputs() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if len(outcomes) != 1113 or outcomes.event_id.duplicated().any():
    ...
    if windows.event_id.nunique() != 1113:
...
def causal_path_features(frame: pd.DataFrame, signal_idx: int) -> dict[str, float]:
    valid = frame.loc[
        frame.current_valid.fillna(False)
        & frame[["coord_open", "coord_high", "coord_low", "coord_close"]]
        .notna().all(axis=1)
    ].sort_values("cal_idx", kind="mergesort")
    prior60 = valid.loc[valid.cal_idx.lt(signal_idx)].tail(60)
    prior20 = prior60.tail(20)
    prior10 = prior60.tail(10)
    if len(prior60) < 60 or len(prior20) < 20 or len(prior10) < 10:
        return {
            "pre60_position": math.nan,
            "pre20_range": math.nan,
            "pre20_efficiency": math.nan,
            "pre10_return": math.nan,
            "pre10_down_volume_share": math.nan,
            "pre20_large_up_days": math.nan,
        }
    high60 = float(prior60.coord_high.max())
    low60 = float(prior60.coord_low.min())
    last_close = float(prior60.iloc[-1].coord_close)
    first20_close = float(prior20.iloc[0].coord_close)
    path_move = float((prior20.coord_close.diff().abs()).sum())
    net_move = abs(last_close - first20_close)
    down_turn = float(prior10.loc[prior10.step_return.lt(0), "turnover_fraction"].sum())
    all_turn = float(prior10.turnover_fraction.sum())
    return {
        "pre60_position": (
            (last_close - low60) / (high60 - low60) if high60 > low60 else math.nan
        ),
        "pre20_range": float(prior20.coord_high.max() / prior20.coord_low.min() - 1),
        "pre20_efficiency": net_move / path_move if path_move > 0 else math.nan,
        "pre10_return": float(last_close / prior10.iloc[0].coord_close - 1),
        "pre10_down_volume_share": down_turn / all_turn if all_turn > 0 else math.nan,
        "pre20_large_up_days": float(prior20.step_return.ge(0.05).sum()),
    }
...
def build_ledger(
    ...
    merged = candidates.merge(
        outcomes[
            [
                ...
                "exit_date", "exit_price", "exit_reason", "holding_sessions", "net_return",
            ]
        ],
        on="event_id",
        how="left",
    ...
    for event in ordered.itertuples(index=False):
        features = causal_path_features(
            window_lookup[str(event.event_id)], int(event.signal_cal_idx)
        )
        ...
        rows.append(
            {
                ...
                "lag1_ret60": float(event.lag1_ret60),
                "pullback_depth": float(event.pullback_depth),
                "intraday_excursion": float(event.intraday_excursion),
...
def render_chart(event: pd.Series, frame: pd.DataFrame, output: Path) -> None:
    ...
    price_ax.axvline(signal_x, color="#b45309", linewidth=1.3, label="pullback absorption")
    price_ax.axhline(100.0, color="#b45309", linewidth=0.7, linestyle=":", label="signal close")
    prior = frame.loc[frame.cal_idx.lt(int(event.signal_cal_idx))].tail(10)
    if not prior.empty:
        prior_high = float(prior.coord_high.max()) * 100 / signal_close
        price_ax.axhline(prior_high, color="#2563eb", linewidth=0.8, linestyle="--", label="prior10 high")
    ...
    if math.isfinite(float(event.target_price)):
        price_ax.axhline(float(event.target_price) * 100 / signal_close, color="#f97316", linewidth=0.9, linestyle=":", label="+10% target")
    ...
    price_ax.set_title(
        f"{int(event.chart_number):04d} | {event.symbol} | {signal_date.date()} | BULL DECEL | {status}\n"
        f"leader60 {float(event.lag1_ret60):+.1%} | pullback {float(event.pullback_depth):.1%} | "
        f"undercut {float(event.intraday_excursion):+.1%} | turn {float(event.signal_turnover_ratio):.2f}x | "
        f"mkt r20/r60 {float(event.market_median_ret20):+.1%}/{float(event.market_median_ret60):+.1%} | "
        f"pre60 pos {float(event.pre60_position):.0%} eff {float(event.pre20_efficiency):.2f}",
        fontsize=9.2,
    )
    ...
    volume_ax.bar(
        turn_dates, valid_turn.turnover_fraction * 100, width=0.75,
    ...
    figure.savefig(
        output, dpi=105, bbox_inches="tight", facecolor="white",
