...
EXPECTED_FEATURE_CONTRACT_HASH = "96fb2f13d04711584b4773b7e9280016f93e7341ccac6ab3a04703f4d70b252b"
...
def _choose_exit(target:dict[str,Any]|None,stop:dict[str,Any]|None,horizon:dict[str,Any]|None) -> tuple[dict[str,Any]|None,str]:
    choices=[]
...
def build_policy_paths(entries:pd.DataFrame,candidates:pd.DataFrame) -> tuple[pd.DataFrame,pd.DataFrame]:
    ...
    for e in unique.itertuples(index=False):
        ...
        h40=horizon[40]; h40net=np.nan if h40 is None else (h40["raw_price"]*(1-COST)+_cash_amount(acts,ed,h40["exit_date"]))/(e.entry_raw_price*(1+COST))-1
        base={"entry_key":e.entry_key,"candidate_id":e.candidate_id,"symbol":e.symbol,"board":e.board,"memory_state":e.memory_state,"entry_time":et,"entry_date":ed,"entry_cal_idx":int(e.entry_cal_idx),"entry_raw_price":float(e.entry_raw_price),"entry_coord_price":float(e.entry_coord_price),"L":float(e.L),"U":float(e.U),"W":float(e.W),"u_time":target_time,"mae40":mae,"y_clean_u20":bool(target is not None and target["cal_idx"]<=int(e.entry_cal_idx)+20 and (pd.isna(loss_times[.08]) or target["exit_time"]<loss_times[.08])),"y_clean_u40":bool(target is not None and target["cal_idx"]<=h40_end and (pd.isna(loss_times[.10]) or target["exit_time"]<loss_times[.10])),"y_tail_failure40":bool((target is None or target["cal_idx"]>h40_end) and ((np.isfinite(h40net) and h40net<=-.10) or mae<=-.20))}
        for x in (.05,.08,.10,.12,.15): base[f"u_before_loss{int(x*100)}"]=bool(target is not None and target["cal_idx"]<=h40_end and (pd.isna(loss_times[x]) or target["exit_time"]<loss_times[x]))
        for d in (3,5,10):
        ...
        for H in TIME_STOPS:
            for policy in policies:
                chosen,why=_choose_exit(target if target is not None and target["cal_idx"]<=int(e.entry_cal_idx)+H else None,static_stops[policy],horizon[H])
...
def main() -> None:
    ...
    parser.add_argument("stage", choices=("preflight", "stage-a", "verify-freeze", "stage-b"))
    ...
    if args.stage == "preflight":
        ...
        print(json.dumps(clean({"readiness":readiness,"minute_audit":minute_audit,"entry_status":summary.to_dict("records"),"outcomes_opened":"NO"}),indent=2))
    else:
