...
def run_post_observation() -> dict[str,Any]:
    ...
    return {"folds":len(sf),"cash_folds":int((~sf.selected).sum()),"trade_rows":len(tf),"post_observation_label":"INVALID_POST_OBSERVATION_DIAGNOSTIC_EARLY_POOLED_OUTCOME_DISCLOSURE","procedure_changed_between_2022_2023_count":0}
...
def finalize_outputs() -> dict[str,Any]:
    ...
    lines += ["","MAIN fixed complete candidates admitted zero TEST signals because every TEST score fell below the causal TRAIN A20 cutoff. ChiNext candidates had only 28 trades; this is far below the frozen minimum evidence requirement.","","## Entry diagnostics artifact","",f"All 35 entry translations with raw/eligible/executable/missed/no-confirmation/delay/price/outcome fields: `{ENTRY_DIAGNOSTICS}`.","","## Mechanism findings","",
      ...
      "","## 2022–2023","","The exact procedure later selected CASH for Main and ChiNext in both years, but **no 2022–2023 scientific evidence is accepted**. Before the Development procedure freeze, an all-years pooled X0 outcome summary that included 2022–2023 was displayed. No year-specific result was then inspected, no selection used those rows, and no rule changed, but the required chronology was nonetheless breached; the later 2022–2023 run is invalid as a post-observation robustness diagnostic.","","## Audit","","```json",json.dumps(clean(audit),indent=2,sort_keys=True),"```","","## Artifacts","",f"Large artifacts: `{EXT}`. Compact result: `{RESULT}`."]
