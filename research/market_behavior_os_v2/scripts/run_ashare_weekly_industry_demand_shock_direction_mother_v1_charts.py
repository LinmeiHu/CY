#!/usr/bin/env python3
"""Render every frozen weekly industry demand-shock event for chart review."""
...
STAGE_B_RUNNER = REPO / (
    "research/market_behavior_os_v2/scripts/"
    "run_ashare_weekly_industry_demand_shock_direction_mother_v1_stage_b.py"
)
...
EXPECTED_HASHES = {
    ...
    STAGE_B_RUNNER: "a2fed8deec826ff26d743e6c22ea4b41a587ae00a3f08633c4829d9c30798c50",
    ...
    PATHS: "bd236fd6949fb184c3817fb2dc239d97174048aebb6a189fb781564595593d25",
    OUTCOMES: "02093372425e0f7108532a27b7ea880baf898db1ef2182c64212db0e6decd20b",
}
...
def load_inputs() -> tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame]:
    ...
    if len(candidates) != 590 or candidates.event_id.duplicated().any():
        raise ResearchError("candidate identity drift")
    if len(outcomes) != 590 or outcomes.event_id.duplicated().any():
        raise ResearchError("outcome identity drift")
    regime = pd.read_parquet(
        REGIME,
        columns=[
            "trade_date", "market_regime", "market_median_ret20",
            "market_positive_ret20_share", "market_median_ret60",
            "market_positive_ret60_share", "latest_source_timestamp",
        ],
    )
    regime["trade_date"] = pd.to_datetime(regime.trade_date)
    regime["latest_source_timestamp"] = pd.to_datetime(regime.latest_source_timestamp)
    ...
    windows["available_at"] = pd.to_datetime(windows.available_at)
    windows["decision_at"] = pd.to_datetime(windows.decision_at)
    if windows.event_id.nunique() != 590:
    ...
    if candidates.latest_source_timestamp.gt(candidates.decision_at).any():
        raise ResearchError("causal market state arrived after decision")
    if candidates[["market_regime","ret20","ret60"]].isna().any().any():
        raise ResearchError("missing required chart annotation")
...
def build_ledger(
    ...
    for event in candidates.sort_values(
        ["signal_date","direction_lane","symbol","event_id"],kind="mergesort"
        ...
        rows.append({
            ...
            "industry_score": float(event.industry_score),
            "industry_n": int(event.industry_n),
            ...
            "signal_turnover_ratio": (
                ...
                if prior20_turn>0 else math.nan
            ),
            ...
            "pre126_position": (
                ...
                if prior126_high>prior126_low else math.nan
            ),
...
def render_chart(event: pd.Series,frame: pd.DataFrame,output: Path) -> None:
    ...
    if pd.notna(event.entry_date) and math.isfinite(float(event.entry_price)):
        ...
        price_ax.scatter(
            entry_x,float(event.entry_price)*100/signal_close,marker="^",s=65,
            color="#7b2cbf",zorder=8,label="entry",
        )
        price_ax.axhline(
            float(event.entry_price)*1.10*100/signal_close,color="#ff7f0e",
            linewidth=0.75,linestyle="--",label="+10% target",
        )
    if pd.notna(event.exit_date) and math.isfinite(float(event.exit_price)):
        ...
        price_ax.scatter(
            exit_x,float(event.exit_price)*100/signal_close,marker="v",s=65,
            color="#111111",zorder=8,label="exit",
        )
    ...
    price_ax.set_title(
        f"{int(event.chart_number):04d} | {event.direction_lane} | {event.symbol} | "
        f"{signal_date.date()} | {event.market_regime} | {status}\n"
        f"industry shock {float(event.industry_score):+.1%} | stock {float(event.stock_step_return):+.1%} | "
        f"r20/r60 {float(event.ret20):+.1%}/{float(event.ret60):+.1%} | "
        f"turn/20med {float(event.signal_turnover_ratio):.2f}x | "
        f"6m position {float(event.pre126_position):.0%}",
        fontsize=9.8,
    )
    ...
    volume_ax.axvline(signal_x,color="#b45309",linewidth=1)
...
def run(workers: int,skip_render: bool) -> dict[str,Any]:
    ...
    if not skip_render:
        ...
        for _,event in ledger.iterrows():
            target = CHART_DIR / (
                f"{int(event.chart_number):04d}_{event.direction_lane.lower()}_"
