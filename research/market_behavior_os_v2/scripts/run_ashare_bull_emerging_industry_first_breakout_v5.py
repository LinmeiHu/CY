#!/usr/bin/env python3
"""Causal bull emerging-industry first-breakout strategy research."""
...
EXPERIMENT = "ASHARE-BULL-EMERGING-INDUSTRY-FIRST-BREAKOUT-V5"
EXT = Path("/Volumes/quant/CY_quant_research/ashare_bull_emerging_industry_first_breakout_v5")
...
def contract_value() -> dict[str, Any]:
    return {
        ...
        "economic_hypothesis": (
            "In a causal broad bull market whose short breadth is at least its "
            "long breadth, an industry that moves from the lower half to the top "
            "30% of twenty-session performance can represent fresh capital rotation. "
            "A stock's first twenty-session high breakout on expanded turnover is "
            "the simple executable expression of that rotation."
        ),
        "market": {
            "state": v1.contract_value()["market_regime"],
            "expansion": "market positive-ret20 share >= positive-ret60 share",
        },
        "industry": {
            "majority": "positive-ret20 share > 0.50",
            "emergence": "ret20 percentile >=0.70 and same-industry t-5 percentile <=0.50",
        },
        "stock": {
            "first_breakout": (
                "current close >= prior20 high and previous close < its then-prior20 high"
            ),
            "base": "prior20 high/low range <=30%; pre-signal ret20 <=30%",
            "demand": "signal turnover >= prior20 mean",
        },
        "rules": {
            "R1_EMERGING_LEADER": "base contract",
            "R2_TOP20_INDUSTRY": "R1 and industry percentile>=0.80",
            "R3_LOW_EXTENSION": "R1 and pre-signal ret20<=0.15",
            "R4_HIGH_DEMAND": "R1 and turnover expansion>=1.50",
        },
        "profiles": PROFILES,
...
def persist_contract() -> dict[str, str]:
    ...
    v1.write_json(
        ...
        {
            "experiment": EXPERIMENT,
            "status": "OUTCOME_BLIND_EMERGING_INDUSTRY_BREAKOUT_FREEZE",
            ...
            "v4_selection_dependency_sha256": v1.sha256(Path(v4.__file__)),
            ...
            COPY (
              WITH da AS (
                ...
                WHERE trade_date>=DATE '2014-01-01' AND history_n20=20
                  ...
                  AND r.market_positive_ret20_share>=r.market_positive_ret60_share
                  AND i0.industry_n20>=5 AND i0.industry_positive_ret20_share>0.50
                  AND i0.industry_ret20_percentile>=0.70
                  AND i0.industry_percentile_tminus5<=0.50
              ...
              SELECT *,'BROT-'||strftime(trade_date,'%Y%m%d')||'-'||symbol AS event_id,
                ...
                TRUE AS pass_R1_EMERGING_LEADER,
                industry_ret20_percentile>=0.80 AS pass_R2_TOP20_INDUSTRY,
...
def sample_and_render(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    sample.insert(0, "chart_id", [f"BULL-ROTATE-{i:03d}" for i in range(1, 41)])
...
def persist_failure(table: pd.DataFrame, reason: str) -> dict[str, Any]:
    ...
    result = {
        "experiment": EXPERIMENT,
        "verdict": "BULL_EMERGING_INDUSTRY_BREAKOUT_FAILS_DISCOVERY_SUPPORT",
        "reason": reason,
...
def run_stage_b() -> dict[str, Any]:
    ...
    if selected is None:
        return persist_failure(
            table,
            "No frozen rule/profile has required discovery support and chronology",
        )
    ...
    result = {
        ...
        "verdict": "BULL_EMERGING_INDUSTRY_BREAKOUT_EDGE"
        if all(gate.values())
        else "BULL_EMERGING_INDUSTRY_BREAKOUT_FAILS_TARGET",
        "hashes": {
