#!/usr/bin/env python3
"""Render every frozen within-industry circulating-size extreme for visual review."""
...
STAGE_B_RUNNER = REPO / (
    "research/market_behavior_os_v2/scripts/"
    "run_ashare_industry_neutral_circulating_size_extremes_mother_v1_stage_b.py"
)
...
OUTPUT_ROOT = DATA_ROOT / "ashare_industry_neutral_circulating_size_extremes_mother_v1"
...
EXPECTED_HASHES = {
    ...
    STAGE_B_RUNNER: "e446be9155d7af05bc8f026d42c39dd879732de2d6990db2ca6115292547a7f8",
    ...
    STAGE_B_MANIFEST: "c05a54cfde948619bc7c5512b86db65b071b2edf51c0b55763564723a9843d93",
    PATHS: "3cd09c1cf6d9f026ea60f98b9b9e94631abf7fad034c21526e16b3db4c257137",
    OUTCOMES: "9980d3ba37b7958f96b0e32c7735fb00d67bccb02d02256c636e9bcfe40af97e",
}
...
def load_inputs() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    ].apply(pd.to_datetime).max(axis=1)
        ...
        rows.append(
            {
                ...
                "size_lane": str(event.size_lane),
                "pit_industry": str(event.pit_industry),
                ...
                "market_positive_ret20_share": charts.safe_float(
                    event.market_positive_ret20_share
                ),
                "market_positive_ret60_share": charts.safe_float(
                    event.market_positive_ret60_share
                ),
                "size_positive_participation_small30_large30": charts.safe_float(
                    event.size_positive_participation_small30_large30
                ),
                "size_leadership_transition5": charts.safe_float(
                    event.size_leadership_transition5
                ),
                "circulating_market_value_cny": float(
                    event.circulating_market_value_cny
                ),
                ...
                "pre126_return": (
                    signal_close / float(pre.coord_close.iloc[0]) - 1.0
                    if not pre.empty
                    else math.nan
                ),
                "pre126_position": (
                    (signal_close - prior_low) / (prior_high - prior_low)
                    if math.isfinite(prior_high) and prior_high > prior_low
                    else math.nan
                ),
...
def render_chart(event: pd.Series, frame: pd.DataFrame, output: Path) -> None:
    ...
    price_ax.axvline(signal_x, color="#b45309", linewidth=1.4, label="size event")
    price_ax.axhline(
        signal_high, color="#b45309", linewidth=0.8, linestyle="--", label="event high"
    )
    price_ax.axhline(
        signal_low, color="#2563eb", linewidth=0.8, linestyle="--", label="event low"
    )
    ...
    if pd.notna(event.h120_exit_date) and math.isfinite(float(event.h120_exit_price)):
        ...
        price_ax.scatter(
            x,
            float(event.h120_exit_price) * 100 / signal_close,
            marker="v",
            s=62,
            color="#111111",
            zorder=8,
    ...
    price_ax.set_title(
        ...
        f"{event.size_lane} | {event.pit_industry} | {event.market_regime} | {status}\n"
        f"circulating size {float(event.circulating_market_value_cny) / 1e9:.2f}bn CNY | "
        f"turn {float(event.signal_turnover_fraction):.1%} | "
        f"event ret {float(event.signal_step_return):+.1%} | "
        f"pre r20/r60/r120 {float(event.ret20):+.1%}/{float(event.ret60):+.1%}/"
        f"{float(event.ret120):+.1%} | H20/H60 {float(event.h20_net_return):+.1%}/"
        f"{float(event.h60_net_return):+.1%} | MFE/MAE "
        f"{float(event.h120_mfe):+.1%}/{float(event.h120_mae):+.1%}",
        fontsize=8.6,
    )
    ...
    price_ax.legend(
        by_label.values(), by_label.keys(), loc="upper left", ncol=5, fontsize=6.7
    )
...
def run(workers: int, skip_render: bool) -> dict[str, Any]:
    ...
    if not skip_render:
        ...
        contact_sheets = build_nine_up(
            index, SHEET_DIR, f"{EXPERIMENT} CHRONOLOGICAL"
        )
        for bucket in (
            "SEVERE_LOSS",
            "LOSS_0_TO_10PCT",
