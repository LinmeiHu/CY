#!/usr/bin/env python3
"""Attach the one frozen V36 development lifecycle after CY-057 authorization."""
...
RUNNER = SCRIPTS / "run_ashare_broad_market_price_delay_compensation_mother_v36_stage_b.py"
...
STAGE_A_MANIFEST = EXPERIMENTS / "ASHARE-V36-CY054_DATA_ASSET_MANIFEST.json"
STAGE_B_MANIFEST = EXPERIMENTS / "ASHARE-V36-CY057_DATA_ASSET_MANIFEST.json"
REGISTRY_SUGGESTION = EXPERIMENTS / "ASHARE-V36-CY057_REGISTRY_SUGGESTION.json"
PHASE1_FREEZE_SPEC = EXPERIMENTS / "ASHARE-V36_phase1_ledger_freeze_spec.json"
PHASE1_FINALIZER = SCRIPTS / (
    "finalize_ashare_broad_market_price_delay_compensation_mother_v36_phase1.py"
)
PHASE1_RELIABILITY = EXPERIMENTS / "ASHARE-V36_phase1_axis_reliability_decision.json"
...
PARTITIONS = tuple(
    CY006_ROOT / f"partition_year={year}/data_0.parquet" for year in range(2019, 2022)
)
...
PHASE1_MANIFEST = OUTPUT_ROOT / "stage_d_phase1_anonymous_review/manifest.json"
PHASE1_LEDGER = OUTPUT_ROOT / "stage_d_phase1_anonymous_review/phase1_annotation_ledger.csv"
STAGE_B = OUTPUT_ROOT / "stage_b"

ASSET_ID = "CY-057"
AUTHORIZATION_ID = "CYQ-AUTH-ASHARE-BROAD-MARKET-PRICE-DELAY-V36-STAGE-B-2019-2021Q3-V1"
AUTHORIZATION_PURPOSE = "ASHARE_FROZEN_DEVELOPMENT_OUTCOME_ATTACHMENT"
AUTHORIZED_ARM = "V36_FROZEN_STAGE_B_OUTCOME_ATTACHMENT_ONLY"
...
EXPECTED_CANDIDATES = 531
EXPECTED_ANNUAL = {2019: 249, 2020: 282}
YEARS = (2019, 2020)
STAGE_B_SPEC_SHA256 = "c3867b272c05c5e558b56abe6097aa5a7ee6b3bca5c649d577643e0446f47655"

EXPECTED_INPUTS: dict[str, tuple[Path, str]] = {
    "parent_spec": (
        PARENT_SPEC,
        "2dc6e2b1ac6e29105172cddc8a6d5dbafa06b30645a8f48b08abdf8f348286fc",
    ),
    "stage_b_spec": (STAGE_B_SPEC, STAGE_B_SPEC_SHA256),
    "stage_a_runner": (
        STAGE_A_RUNNER,
        "3cf20dead4f7a04b1e8981502d484fd1e56599747129f3ff92085a462c0edf51",
    ),
    "stage_a_manifest": (
        STAGE_A_MANIFEST,
        "8466e1ccd6cbbff159beb4ee9d582a0917f7eba9015d7a76251f273727e4973c",
    ),
    "stage_a_result": (
        STAGE_A_RESULT,
        "5d05d21c78f5c4722cdcf3eb86db53e0d9d37d7a37e45a465b1ee78a924d8529",
    ),
    ...
    "stage_a_candidates": (
        CANDIDATES,
        "6f9e84f1828bb054d8baea187e72563c8454909fe71f3a932ee8ea5456ff4e0b",
    ),
    "phase1_freeze_spec": (
        PHASE1_FREEZE_SPEC,
        "4ee8ef1b2912bebd857c3fbb19b9f9d94227f5f16c15583a81bf1c7f8a53f390",
    ),
    "phase1_finalizer": (
        PHASE1_FINALIZER,
        "c1fe1d2fe3f48f4d55878348d0125c7a34d3461a6b77f8530ccc17a8781d68ea",
    ),
    "phase1_reliability": (
        PHASE1_RELIABILITY,
        "6f5c76d0abcb0ad2e7836f32ae48587036ffa5c5a585919996b6d0370f3ea1e6",
    ),
    "phase1_manifest": (
        PHASE1_MANIFEST,
        "7a3133223e5aecf99ecf33d843c31f18d726d927810a46f54f5863027005bc7b",
    ),
    "phase1_ledger": (
        PHASE1_LEDGER,
        "dafc4594cb502f50e22946f1206f147b1083d0e41f849b13bc32e058b46c36a6",
    ),
    "cy006_manifest": (
        CY006_MANIFEST,
...
def public_expected_declarations() -> dict[str, dict[str, Any]]:
    return {
...
def verify_public_envelope() -> dict[str, str]:
    for path in (
        *[item[0] for item in EXPECTED_INPUTS.values() if item[0].is_relative_to(REPO)],
        RUNNER,
        STAGE_B_MANIFEST,
        REGISTRY_SUGGESTION,
        REGISTRY,
    ...
    manifest = load_json(STAGE_B_MANIFEST, "CY-057 manifest")
    suggestion = load_json(REGISTRY_SUGGESTION, "CY-057 registry suggestion")
    ...
    if set(source) | set(lineage) != set(declared) or set(source) & set(lineage):
        raise ResearchError("CY-057 manifest role partition drift")
    for role, item in {**source, **lineage}.items():
        if (
            item.get("path") != declared[role]["path"]
            or item.get("sha256") != declared[role]["sha256"]
        ):
            raise ResearchError(f"CY-057 manifest binding drift: {role}")
    ...
    if (
        ...
        raise ResearchError("CY-057 manifest semantics drift")
    ...
    if not isinstance(suggested_asset, dict) or not isinstance(suggested_auth, dict):
        raise ResearchError("CY-057 suggestion structure drift")
    ...
    if asset != suggested_asset or auth != suggested_auth:
        raise ResearchError("installed CY-057 registry objects differ from frozen suggestion")
    ...
    if (
        ...
        raise ResearchError("installed CY-057 lineage/protocol drift")
    bound = role_map(auth.get("bound_artifacts"), "CY-057 bound artifacts")
    if set(bound) != set(declared):
        raise ResearchError("CY-057 bound-artifact role set drift")
    for role, expected in declared.items():
        if (
            bound[role].get("path") != expected["path"]
            or bound[role].get("sha256") != expected["sha256"]
        ):
            raise ResearchError(f"CY-057 authorization binding drift: {role}")
    return {
        "runner": runner_hash,
...
def verify_inputs() -> tuple[dict[str, str], dict[str, Any], dict[str, str]]:
    ...
    for role, (path, expected) in EXPECTED_INPUTS.items():
        ...
        actual[role] = value
    stage_a = load_json(STAGE_A_RESULT, "V36 Stage-A result")
    if (
        ...
        or stage_a.get("representation_sha256") != EXPECTED_INPUTS["stage_a_representation"][1]
    ):
        raise ResearchError("V36 Stage-A result does not authorize Stage B")
    phase1 = load_json(PHASE1_MANIFEST, "V36 frozen Phase-1 manifest")
    if (
        phase1.get("status") != "FROZEN_OUTCOME_BLIND"
        or phase1.get("events") != EXPECTED_CANDIDATES
        or phase1.get("coverage_exactly_once") is not True
        or phase1.get("eligible_discovery_prompt_axes") != ["primary_morphology", "market_tape"]
        or phase1.get("descriptive_only_axes") != ["signal_candle", "turnover_state"]
        or phase1.get("governance", {}).get("outcomes_read") is not False
        or phase1.get("governance", {}).get("rule_aggregation_performed") is not False
    ):
        raise ResearchError("V36 Phase-1 freeze does not authorize Stage B")
...
def prepare_candidates(
    ...
    prepared = connection.execute(
        ...
        SELECT c.* EXCLUDE(signal_cal_idx),c.signal_cal_idx AS source_signal_cal_idx,
          ...
          CAST('PRICE_DELAY' AS VARCHAR) AS sleeve,
        ...
        WHERE c.signal_date BETWEEN DATE '{SIGNAL_START.date()}' AND DATE '{SIGNAL_END.date()}'
          AND c.selection_rank=1 AND c.delay_status='VALID'
          AND isfinite(c.price_delay_52w_4l)
          AND c.price_delay_52w_4l BETWEEN 0 AND 1
          ...
          AND NOT r.corporate_action_blocking AND r.market_rule_valid
          AND r.historical_identity_valid AND r.current_day_data_tradable
          AND r.available_at<=r.decision_at
    ...
    audit = {
        ...
        "invalid_delay_rows": int(
            (
                ~np.isfinite(prepared.price_delay_52w_4l)
                | ~prepared.price_delay_52w_4l.between(0, 1)
            ).sum()
        ),
    }
    zero_keys = (
...
def build_paths(
    ...
    query = f"""
      ...
      SELECT r.event_id,r.symbol,r.event_signal_date,r.event_signal_cal_idx,
        ...
        r.low*a.adjusted_close/r.close AS coord_low,a.adjusted_close AS coord_close,
    ...
    audit = {
        ...
        "after_plus126_rows": int(paths.cal_idx.gt(paths.event_signal_cal_idx + 126).sum()),
    }
    if (
        audit["minimum_calendar_sessions_after_latest_signal"] < 126
        or audit["events_with_rows"] != EXPECTED_CANDIDATES
        or audit["events_without_exact_126_slots"]
        or audit["duplicate_event_cal_idx"]
        or audit["post_cap_rows"]
        or audit["after_plus126_rows"]
    ):
...
def summarize(
    ...
    return {
        ...
        "next_action": (
            "FREEZE_AND_RENDER_ALL_531_DEVELOPMENT_OUTCOME_CHARTS; "
            "KEEP_2022_2024_CLOSED_UNLESS_A_LATER_FROZEN_RULE_PASSES_BOTH_ANNUAL_GATES"
        ),
    }
...
def run() -> dict[str, Any]:
    ...
    if STAGE_B.exists() or STAGE_B.is_symlink():
        raise ResearchError(f"canonical V36 Stage B already exists: {STAGE_B}")
    ...
    try:
        ...
        if verify_inputs()[0] != source_hashes:
            raise ResearchError("bound source changed during V36 Stage B")
        if verify_public_envelope() != public_hashes:
            raise ResearchError("public authorization changed during V36 Stage B")
