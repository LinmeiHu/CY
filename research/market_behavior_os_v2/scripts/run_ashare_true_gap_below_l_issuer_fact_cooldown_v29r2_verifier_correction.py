#!/usr/bin/env python3
"""Verify immutable CY-063 Stage A canonically, then run one CY-064 Stage B."""
...
PREDECESSOR_ASSET_ID = builder.PREDECESSOR_ASSET_ID
PREDECESSOR_AUTHORIZATION_ID = builder.PREDECESSOR_AUTHORIZATION_ID
...
PROTOCOL_ROLE = "stage_a_verifier_correction_r3_freeze"

EXPECTED_AUTH_ALLOWED_USES = [
    (
        "verify the immutable CY-063 Stage-A seven-file cohort using only the narrowly "
        "bounded serialization-representation comparator and freeze one outer verification"
    ),
    (
        "after independent outer-freeze SHA review, run one unchanged A67 H20 40-bp K80 "
        "Stage-B replay in the exact new output root under a read-only pre-outcome seal"
    ),
]
EXPECTED_AUTH_BLOCKED_USES = [
    (
        "materializing or selecting a new Stage-A cohort; copying, deleting, overwriting, "
        "resuming or mutating CY-063 Stage-A artifacts; or broad dtype/null normalization "
        "beyond the two frozen representations"
    ),
    (
        "changing source, causal, title taxonomy, cooldown, selector, threshold, execution, "
        "portfolio or reporting semantics"
    ),
    "alternate roots, repeated Stage-B attempts, strict PIT-A, pristine-OOS or live claims",
]


class VerifierCorrectionError(RuntimeError):
    """Fail closed on registration, immutable-stage, or Stage-B boundary drift."""


def require(condition: bool, message: str) -> None:
    if not condition:
        raise VerifierCorrectionError(message)
...
def read_json(path: Path, label: str) -> dict[str, Any]:
    ...
    except builder.VerifierCorrectionAssetError as exc:
        raise VerifierCorrectionError(str(exc)) from exc
...
def verify_registration(asset_root: Path) -> dict[str, Any]:
    ...
    predecessors = [
        ...
        if item.get("asset_id") == PREDECESSOR_ASSET_ID
    ]
    predecessor_auths = [
        item
        for item in registry.get("bounded_authorizations", [])
        if item.get("authorization_id") == PREDECESSOR_AUTHORIZATION_ID
    ]
    require(
        len(assets) == len(auths) == len(predecessors) == len(predecessor_auths) == 1,
        "CY-064 or immutable CY-063 registration is missing",
    )
    ...
    require(
        ...
        "CY-064 registry semantics drifted",
    )
    ...
    require(
        ...
        "CY-064 registered coverage/output roots drifted",
    )
    ...
    require(
        ...
        "CY-064 dependency/scope drifted",
    )
    require(
        ...
        and auth.get("verifier_only_correction") is True
    ...
    require(
        ...
        "CY-064 exact artifact role set drifted",
    )
    for role, item in fingerprints.items():
        ...
        require(
            ...
            f"CY-064 artifact/manifest fingerprint drifted: {role}",
        )
    protocol_fact = inventory.get(PROTOCOL_ROLE)
    require(protocol_fact is not None, "CY-064 manifest lacks protocol")
    expected_protocol = {
    ...
    require(
        ...
        "CY-064 protocol/runner binding drifted",
    )
    require(
        auth.get("allowed_uses") == EXPECTED_AUTH_ALLOWED_USES
        and auth.get("blocked_uses") == EXPECTED_AUTH_BLOCKED_USES,
        "CY-064 allowed/blocked use contract drifted",
    )
    predecessor_identity = manifest.get("predecessor", {}).get("registry_identity", {})
    require(
        ...
        "CY-063 predecessor registry identity drifted",
    )
    return {
        "asset": asset,
        "authorization": auth,
...
def verify_asset(asset_root: Path, parent_selected: Path) -> dict[str, Any]:
    require(asset_root.resolve() == EXPECTED_ASSET_ROOT, "unexpected CY-064 asset root")
    registered = verify_registration(asset_root)
    try:
        validation = builder.validate(asset_root)
    except builder.VerifierCorrectionAssetError as exc:
        raise VerifierCorrectionError(f"CY-064 wrapper validation failed: {exc}") from exc
    manifest_path = asset_root / "asset_manifest.json"
    manifest = read_json(manifest_path, "CY-064 manifest")
    require(
        ...
        "CY-064 wrapper or parent-selected identity drifted",
    )
    return {
        "asset_id": ASSET_ID,
        ...
        "predecessor_cy063_identity": validation["stage_a_verification"]["cy063_identity"],
        "cy063_stage_a_verification": validation["stage_a_verification"],
        "bound_artifacts": {
...
def outer_freeze_payload(
    ...
    return {
        "stage": "CY064_CY063_STAGE_A_CANONICALLY_VERIFIED_BEFORE_OUTCOME_ACCESS",
        "created_at": created_at,
        "verifier_correction_wrapper_identity": identity,
        "cy063_stage_a_root": str(CY063_OUTPUT_ROOT),
        "cy063_stage_a_freeze": {
            "path": str(CY063_OUTPUT_ROOT / "stage_a/freeze.json"),
            "sha256": builder.CY063_STAGE_HASHES["freeze.json"],
        },
        "cy063_stage_a_artifact_hashes": dict(builder.CY063_STAGE_HASHES),
...
def run_verify_only(
    ...
    require(
        ...
        "CY-064 verifier requires the frozen verification root",
    )
    require(
        stage_b_root.resolve() == EXPECTED_STAGE_B_ROOT,
        "CY-064 verifier requires the frozen Stage-B root",
    )
    require(not verification_root.exists(), "refusing non-pristine CY-064 verification root")
    require(not stage_b_root.exists(), "CY-064 Stage-B root must remain absent during verification")
    ...
    verification_root_created = False
    try:
        verification_root.mkdir(parents=False, exist_ok=False)
        verification_root_created = True
        ...
        require_read_only(verification_root, "CY-064 verification root")
        require_read_only(freeze_path, "CY-064 outer freeze")
        require(
            read_json(freeze_path, "CY-064 outer freeze") == freeze,
            "CY-064 outer freeze did not round-trip exactly",
        )
        return {
            "verified": True,
            ...
            "rejected_signals": stage["rejected_signals"],
            "selected_by_signal_year": stage["selected_by_signal_year"],
            "stage_b_root_absent": not stage_b_root.exists(),
        }
    finally:
        if verification_root_created:
            _seal_completed_tree(verification_root)
...
def verify_outer_freeze(
    ...
    require(
        verification_root.resolve() == EXPECTED_VERIFICATION_ROOT,
        "unexpected CY-064 verification root",
    )
    builder.exact_top(verification_root, {"verified_stage_a_freeze.json"})
    require_read_only(verification_root, "CY-064 verification root")
    freeze_path = verification_root / "verified_stage_a_freeze.json"
    require_read_only(freeze_path, "CY-064 outer freeze")
    freeze = read_json(freeze_path, "CY-064 outer freeze")
    identity = verify_asset(asset_root, parent_selected)
    expected = outer_freeze_payload(
        identity, identity["cy063_stage_a_verification"], str(freeze.get("created_at"))
    )
    require(freeze == expected, "CY-064 outer freeze semantic reconstruction failed")
    return {
        "verified": True,
        ...
        "selected_signals": identity["cy063_stage_a_verification"]["selected_signals"],
    }
...
def _complete_stage_b_after_seal(
    *,
    ...
    """Execute only after the immutable first-file attempt seal exists."""
    ...
    require(
        ...
        "CY-064 outer freeze or CY-063 Stage-A bytes drifted during Stage B",
    )
    ...
    result = {
        "strategy": "V29R2-CY064-VERIFIER-ONLY-CORRECTION",
        "status": "COMPLETE_PIT_B_FIXED_RULE_ROLLFORWARD_VERIFIER_ONLY_CORRECTION",
        "scientific_periods": {
        ...
        "evidence_grade": "PIT_B_CURRENT_OFFICIAL_ENUMERATION_REVISION_HISTORY_INCOMPLETE",
        "verifier_correction_wrapper_identity": identity,
        "predecessor_cy063_identity": identity["predecessor_cy063_identity"],
        ...
        "in_memory_title_classification_verification_replay_performed": True,
        "rule_changed": False,
        "verifier_representation_correction_only": True,
        "v29r1_later_period_status": "BLOCKED_BY_IMMUTABLE_SEMANTIC_INVALIDITY",
    }
...
def run_stage_b(
    ...
    require(output_root.resolve() == EXPECTED_STAGE_B_ROOT, "unexpected CY-064 Stage-B root")
    require(not output_root.exists(), "refusing repeated/non-pristine CY-064 Stage-B attempt")
    ...
    for role, path in supplied.items():
        ...
        require(
            Path(str(fact["path"])).resolve() == path.resolve()
            and fact["sha256"] == sha256(path),
            f"CLI parent {role} differs from CY-064 authorization",
        )
    selected_path = CY063_OUTPUT_ROOT / "stage_a/selected_entries.parquet"
    require(
        sha256(selected_path) == builder.CY063_STAGE_HASHES["selected_entries.parquet"],
    ...
    try:
        ...
        attempt = {
            "stage": "CY064_STAGE_B_EXCLUSIVE_PRE_OUTCOME_ATTEMPT_SEAL",
        ...
        require_read_only(attempt_path, "CY-064 pre-outcome attempt seal")
        require(
            read_json(attempt_path, "CY-064 pre-outcome attempt seal") == attempt,
            "CY-064 pre-outcome attempt seal did not round-trip exactly",
        )
