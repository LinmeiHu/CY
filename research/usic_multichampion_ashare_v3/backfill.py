...
from .legacy_replay import load_actions

def fetch():
    gaps=[pd.read_csv(p) for p in OUT.glob('*/action_gaps.csv')]
    if not gaps:return
    ...
    for g in needed.to_dict('records'):
        ...
        if not data.get('announcements') and symbol.startswith('60'):
            params=dict(pageNum=1,pageSize=30,column='sse',tabName='fulltext',searchkey={'603392':'万泰生物','603466':'风语筑'}.get(symbol,symbol),seDate=f'{(pd.Timestamp(day)-pd.Timedelta(days=1)).date()}~{day}',isHLtitle='false')
            query=root/f'{symbol}_{day}_name_range.json'
