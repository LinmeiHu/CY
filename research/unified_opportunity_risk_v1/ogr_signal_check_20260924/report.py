...
def main():
    ...
    status=dict(status='MINUTE_GATE_BLOCKED_NON_QMT_SOURCE_MISSING',start='2026-09-17',end='2026-09-23',daily_source='Baostock unadjusted fallback',daily_rows=18565,hard_valid_rows=17855,prior_v13_candidates_preserved=len(prior),new_v13_candidates=10,new_v27_and_daily_quality_survivors=len(out),formal_signals_confirmed=None,minute_requirement='exact 120 trading sessions of 1-minute data per candidate',additional_gates=['independent daily-source overlap','post-2026-09-16 corporate actions'],minute_source_policy='non-QMT only',missing_symbols_with_no_post_september16_bars=json.loads((HERE/'baostock_receipt_complete.json').read_text())['absent_symbols'],input_hashes={str(f):repair.digest(f) for f in files})
