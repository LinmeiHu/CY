...
def load_actions():
    inv=json.loads((INV/'QD-010-cninfo-actions-20260820.json').read_text());frames=[]
    for x in inv['files']:
...
def replay(m,s,frame):
    ...
    for t,day in enumerate(m.dates):
        ...
        for ci,(r,base) in enumerate(chosen):
            ...
            # factor_t depends on prior close and effective known terms, not current close.
...
def run():
    m=Market();summaries=[];frames={};start=time.time()
    for s in scenarios():
        ...
        try:
            ...
            dump(d/'identity.json',dict(config=s,config_hash=s['config_hash'],source_hash=sha(Path(__file__)),input_hash=sha(HERE/'input_binding.json'),features_hash=sha(OUT/(family+'_signals.parquet')),artifacts={p.name:sha(p) for p in d.glob('*.parquet')}))
