"""CA V2 isolated derivative: explicit fractional economics and historical withholding.
All other frozen execution code retained; see source diff.
"""
import pickle
from entitlement_boundary import BranchRequired
...
OUT = OUT / "accounts"
PANEL = OUT.parent / "panel"
OUT.mkdir(exist_ok=True)
@lru_cache(None)
def get(k): return np.load(PANEL / (k + ".npy"), mmap_mode="r")
FRICTION_MULTIPLIER = D(1)
NEW_POSITION_WEIGHT = D('.1')

class AccountGap(Exception):pass
EXECUTION_CHARGE=D(".0005")

def dec(v):return D(str(v))
from costs import fees as historical_fees, dividend_rate, maximum_dividend_rate
START_DATE='2007-01-01'
GROSS=False
def fees(value,day,sell=False,quantity=None,symbol=None):
 return D(0) if GROSS else historical_fees(value,day,sell,quantity,symbol,FRICTION_MULTIPLIER)
def size(budget,limit,day,symbol):
 minimum,increment,maximum=(200,1,100000) if symbol.startswith('688') else (100,100,200000)
 q=min(int(budget/limit)//increment*increment,maximum)
 while q>=minimum and dec(q)*limit+fees(dec(q)*limit,day,quantity=q,symbol=symbol)>budget:q-=increment
 return q if q>=minimum else 0

def allocate_fee(charge,quantities):
 # Apportion existing finite Decimal fee units; never round the physical fee.
 # Integer remainder goes to the last lot, exactly as the frozen allocation contract specifies.
 assert charge>=0 and quantities and all(q>0 for q in quantities)
 sign,digits,exponent=charge.as_tuple();units=int(''.join(map(str,digits)));total=sum(quantities);left=units;result=[]
 for k,q in enumerate(quantities):
  n=left if k==len(quantities)-1 else units*q//total;left-=n
  result.append(D((sign,tuple(map(int,str(n))),exponent)))
 assert left==0 and sum(result,D(0))==charge
 return result
...
def run(policy,signals,market,actions,dates,symbols,forecast_through=None,ranker=None,stagger=False,stagger_scale=D(1), resume_path=None, save_before_t=None, snapshot_path=None, entitlement_branch=None):
 ...
 for t,day in enumerate(dates):
  ...
  try:
   try:
    allocations=ca.resolve_event(rights,t,day,entitlement_branch,lambda j: dec(get('close')[t-1,j])) if entitlement_branch is not None else {}
   ...
   for a in rights:
    ...
    if a['effective'] and not a.get('fractional_cash_paid',True) and day>=str(src['share_credit_date'].date()):
     adjustment=a['fractional_cash_due'];move(t,'FRACTIONAL_ECONOMIC_CASH_APPROXIMATION',adjustment,p['j'],src['event_id'])
     lot_actions.append(dict(t=t,lot_id=p['lot_id'],j=p['j'],event_id=src['event_id'],kind='FRACTIONAL_ECONOMIC_CASH_APPROXIMATION',delta=0,cash_delta=str(adjustment)))
     a['fractional_cash_paid']=True
    if a['effective'] and not a['paid'] and (a['receivable']==0 or day>=str(src['pay_date'].date())):
     ...
     initial=max(D(0),a['tax_base']*ca.initial_tax_rate(str(src['record_date'].date()))-a.get('tax_paid',D(0)))
     if initial:
      move(t,'DIVIDEND_INITIAL_WITHHOLDING',-initial,p['j'],src['event_id']);a['tax_paid']=a.get('tax_paid',D(0))+initial
      a['reserve']=max(D(0),a['reserve']-initial)
      lot_actions.append(dict(t=t,lot_id=p['lot_id'],j=p['j'],event_id=src['event_id'],kind='DIVIDEND_INITIAL_WITHHOLDING',delta=0,cash_delta=str(-initial)))
     ...
     if p['pending']==0:
      ...
      for a in rights:
       if a['position'] is p and not a['settled']:
        liability=a['tax_base']*dividend_rate(acquired,today,str(a['source']['record_date'].date()))-a.get('tax_paid',D(0));assert liability>=0;move(t,'DIVIDEND_TAX',-liability,j,a['source']['event_id']);a['tax_paid']=a.get('tax_paid',D(0))+liability;a['reserve']=D(0);a['settled']=True
   ...
   for q in planned:
    ...
    assert dec(q['q'])*op<=q['decision_nav']*(D('.005') if stagger else D('.1'))
    ...
    p=dict(lot_id=lid,j=j,q=q['q'],initial_q=q['q'],pending=0,signal_t=q['decision_t'],entry_t=t,scheduled_expiry=t+20,due=t+20,entry_price=str(op),entry_cost=str(debit))
   ...
   receivable=sum((a['receivable']+(a.get('fractional_cash_due',D(0)) if not a.get('fractional_cash_paid',True) else D(0)) for a in rights),D(0));free=cash-reserve();navnow=free+mv+receivable
   ...
   available=min(free,navnow*D('.05')*stagger_scale) if stagger else free;slots=10 if stagger else 10-len(positions)
   if t in schedule and t+1<len(dates):
    ...
    for r in candidates.itertuples():
     ...
     budget=min(navnow*(D('.005')*stagger_scale if stagger else NEW_POSITION_WEIGHT),available,dec(np.exp(r.logamount20))*D('.01'))
     if stagger:
      existing=sum(p['q']+p['pending'] for p in positions.values() if p['j']==j)
      budget=min(budget,max(D(0),navnow*D('.1')-dec(existing)*cap))
     qty=size(budget,cap,day,symbols[j])
     if qty==0:continue
