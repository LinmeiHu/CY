#!/usr/bin/env python3
"""Clean execution of H-020 with index volatility sourced exactly once."""
...
SPEC=WORK/'experiments/EXP-RTD-002_spec.json'; DAILY=WORK/'artifacts/daily_regime_features.parquet'; TRADES=WORK/'artifacts/yearly_trades.csv'; PRE=WORK/'artifacts/pre_entry_transitions.csv'
OUT_TABLE=WORK/'artifacts/right_tail_density_incrementality_v2.csv'; OUT_JSON=WORK/'artifacts/right_tail_density_incrementality_v2.json'; REPORT=WORK/'reports/right_tail_density_incrementality_v2.md'
...
def validate()->tuple[dict[str,Any],dict[str,str]]:
 ...
 if s.get('experiment_id')!='EXP-RTD-002' or s.get('status')!='FROZEN_BEFORE_FIRST_INCREMENTALITY_TEST':raise DensityError('invalid spec')
 ids={};bad={}
 ...
 return s,ids
def load()->tuple[pd.DataFrame,dict[str,Any]]:
 tcols=['trade_id','baseline_block','symbol','entry_signal_date','entry_execution_date','mfe','round_trip_return','realized_pnl'];pcols=['trade_id','entry_industry','false_breakout',*RISK];dcols=['baseline_block','trade_date','feature_available_at','first_applicable_trade_date',FEATURE,*NEIGHBORS,*BASE]
 t=pd.read_csv(TRADES,usecols=tcols);p=pd.read_csv(PRE,usecols=pcols);d=pd.read_parquet(DAILY,columns=dcols)
 if len(t)!=399 or t.trade_id.nunique()!=399 or len(p)!=399 or p.trade_id.nunique()!=399:raise DensityError('trade/control key identity changed')
 if len(d)!=1942 or d[['baseline_block','trade_date']].duplicated().any():raise DensityError('daily key identity changed')
 if 'index_realized_vol20' in d.columns or pcols.count('index_realized_vol20')!=1:raise DensityError('index volatility source contract changed')
 t['entry_signal_date']=pd.to_datetime(t.entry_signal_date);t['entry_execution_date']=pd.to_datetime(t.entry_execution_date);d['trade_date']=pd.to_datetime(d.trade_date);d['feature_available_at']=pd.to_datetime(d.feature_available_at);d['first_applicable_trade_date']=pd.to_datetime(d.first_applicable_trade_date)
 f=t.merge(p,on='trade_id',validate='one_to_one').merge(d,left_on=['baseline_block','entry_signal_date'],right_on=['baseline_block','trade_date'],validate='many_to_one')
 iv=[c for c in f.columns if 'index_realized_vol20' in c]
 if len(f)!=399 or f.columns.duplicated().any() or iv!=['index_realized_vol20']:raise DensityError(f'corrected merge contract failed: {iv}')
 causal=(f.feature_available_at.dt.date==f.entry_signal_date.dt.date)&(f.first_applicable_trade_date>f.entry_signal_date)&(f.first_applicable_trade_date<=f.entry_execution_date)
 if not causal.all():raise DensityError('PIT applicability changed')
 f['entry_year']=f.entry_signal_date.dt.year;f['opportunity20']=f.mfe>=.20;f['non_false_breakout']=~f.false_breakout.astype(bool)
 if f[FEATURE].notna().sum()!=387 or f[[FEATURE,*BASE]].notna().all(axis=1).sum()!=383 or int(f.false_breakout.sum())!=213 or f.index_realized_vol20.isna().any():raise DensityError('population/label/missingness changed')
 audit={'trade_rows_before_merge':399,'control_rows_before_merge':399,'daily_rows_before_merge':1942,'trade_key_unique':True,'control_key_unique':True,'daily_block_date_key_unique':True,'join_keys':['trade_id','baseline_block+entry_signal_date=trade_date'],'rows_after_merge':399,'duplicate_columns_after_merge':0,'index_realized_vol20_source':'accepted pre_entry_transitions.csv','index_realized_vol20_columns_after_merge':iv,'index_realized_vol20_missing':0,'causal_rows':399,'feature_complete':387,'fixed_control_complete':383,'false_breakout_count':213}
 return f,audit
