...
`STOP_INTEGRITY_REVIEW_REQUIRED` on 2026-08-30.
...
no strategy or production action is authorized.
...
Human review must decide whether to investigate how EXP-P7-003 outputs were
produced despite the reproducible frozen-gate contradiction or to abandon that
consumed-sample branch. Do not repair EXP-P7-003 in place. A rerun, if explicitly
authorized, needs a new experiment ID, an explicit non-registry compatibility
rule, and fresh isolated outputs.

The preferred scientific continuation is prospective validation of the frozen
breadth-opportunity hypothesis on future untouched PIT-A-quality data, or one new
registered causal data family with a single preregistered incrementality test. Do
not run another 2018-2025 threshold, interaction, overlay, or exit optimization.
