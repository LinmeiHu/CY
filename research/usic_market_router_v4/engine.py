"""V3 cash replay adapted from the hash-bound V2 entitlement engine."""
import json,math,time
from collections import defaultdict
import numpy as np
import pandas as pd
from ..usic_multichampion_ashare_v3.common import HERE,CACHE,sha,dump,parquet,fee,quantity
from .common import OUT
from ..usic_multichampion_ashare_v3.legacy_replay import load_actions,ExecutionGap,metrics
...
def replay(m,s,frame):
    ...
    for t,day in enumerate(m.dates):
        ...
        # V4: states are completed-close information; delay freezes original buy decision.
        v4key=s.get('state_key');v4state='UNCONDITIONAL';state_t=max(0,t-s['delay'])
        v4target=1.;risk_target=1.
        if v4key:
            v4state=str(m.v4state[v4key].iloc[state_t])
            v4target=0. if v4state in s.get('blocked_states',[]) else 1.
            if s.get('budgets'):v4target=float(s['budgets'].get(v4state,v4target))
            risk_state=str(m.v4state[v4key].iloc[max(0,t-1)])
            risk_target=0. if risk_state in s.get('blocked_states',[]) else 1.
            if s.get('budgets'):risk_target=float(s['budgets'].get(risk_state,risk_target))
        if 'fixed_budget' in s:v4target=min(v4target,s['fixed_budget'])
        if v4key or 'fixed_budget' in s:
            allowed_cash=min(allowed_cash,max(0.,v4target*priornav-sum((p['q']+p['pending'])*p['mark'] for p in positions.values())))
        if s.get('book')=='FULL_BOOK' and risk_target==0.:
            for p in positions.values():
                if p['due']>t:
                    p['due']=t;p['planned']=t;p['exit_reason']='STATE_REDUCE_ZERO'
                    audits.append(dict(day=day,t=t,symbol=p['symbol'],lot=p['lot'],type='STATE_REDUCE_INTENT',decision_at=m.dates[t-1]+'T15:00:00+08:00',target=0.,prior_value=(p['q']+p['pending'])*p['mark']))
        chosen=[];free=(1 if s['mode']=='C_ONE' else 10)-len(positions);seen=set()
        for r in candidates:
            j=int(r['j']);base=dict(t=t,signal_t=int(r['t']),symbol=r['symbol'],score=r['score'],setup_id=r['setup_id'],family=r['route'],decision_at=r['decision_at'],state=v4state,state_t=state_t,state_information_cutoff=m.dates[state_t]+'T15:00:00+08:00',mapping_fit_cutoff=r.get('fit_cutoff','STATIC_CONSUMED_DEVELOPMENT'),target=v4target)
            ...
            if allowed_cash<=0:orders.append(dict(base,status='MARKET_BUDGET_BLOCK' if overlay in ['G_DOWN','G_RAMP'] or v4key or 'fixed_budget' in s else 'NO_CASH'));continue
        ...
        for r,base,j,q,limit,U,reserve,unitrisk,isadd in commitments:
            ...
            orders.append(dict(base,status='FILLED',limit=limit,opening=op,fill=price,quantity=q,reserved=reserve,debit=debit,lowopen=op<U,preauction_cash=priorcash))
