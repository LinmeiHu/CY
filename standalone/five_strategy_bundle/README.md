...
Current closure is reported per strategy. OGR, IFCGR, MCB, ATRDR, and the local SMV6 replay are executable from configured raw/registered inputs without frozen signal, trade, event, or NAV inputs. ATRDR is exact through 2026-08-12, including its post-2023 V27 Fast/Slow and accelerating/decelerating Bull routes.
...
OGR and IFCGR reproduce their frozen development reference interval. IFCGR always regenerates its complete OGR parent in the same invocation. SMV6 executes the exact registered source bytes after an internal SHA256 check, emits source callback events, and separately emits cash/100-share-lot fills with the frozen source's commission, slippage, minute-volume limit, and order ordering. ATRDR verifies the two registered QD-010 asset hashes, rebuilds both post-2023 V27 intervals, and returns `FULL_END_TO_END_REPRODUCIBLE`.
