...
class InputSnapshotManifest:
    ...
    def load(
        ...
        *,
        ...
        registry_sha256 = _required_digest(
            payload, "registry_sha256", "input snapshot manifest"
        )
        ...
        try:
            purpose = DataPurpose(
                _required_text(payload, "purpose", "input snapshot manifest")
            )
        ...
        *,
        ...
        if self.purpose is DataPurpose.DATA_PREPARATION:
            if operation is DataOperation.INGEST:
                return self._authorization(operation, registry, software_test)
            raise DataActivationError(
                f"DATA_PREPARATION cannot authorize {operation.value}"
            )
        if operation is DataOperation.INGEST:
            return self._authorization(operation, registry, software_test)
        if not self.hard_valid:
            raise DataActivationError(
                f"hard_valid=false blocks {operation.value}"
            )
        if not all(item.passed for item in self.audits.values()):
            raise DataActivationError(
                f"all causal audits must PASS before {operation.value}"
            )
    ...
    def _validate_semantics(self) -> None:
        ...
        if self.purpose is DataPurpose.SOFTWARE_TEST:
            ...
            if not statuses <= {"DEMO_ONLY", "QA_ONLY"}:
                raise DataActivationError(
                    "SOFTWARE_TEST may bind only DEMO_ONLY or QA_ONLY assets"
                )
            return

        if not self.hard_valid:
            raise DataActivationError(
                f"{self.purpose.value} requires hard_valid=true"
            )
        if not all(item.passed for item in self.audits.values()):
            raise DataActivationError(
        ...
        if not statuses <= {"RESEARCH_CONDITIONAL", "DERIVE_ONLY"}:
            raise DataActivationError(
                f"{self.purpose.value} contains a non-research asset"
            )
        if self.purpose is DataPurpose.STRICT_ARCHIVAL_RESEARCH:
            non_archival = [
...
def _load_binding(
    ...
    *,
    ...
    inventory_sha256 = _optional_digest(
        raw, "inventory_sha256", f"input binding {role}"
    )
    if path.is_file():
        if sha256 is None:
        ...
        if inventory_manifest is not None or inventory_sha256 is not None:
            raise DataActivationError(
                f"file binding {role} must not declare an inventory manifest"
            )
    elif path.is_dir():
        ...
        if not inventory_manifest.is_file():
            raise DataActivationError(
                f"inventory manifest does not exist: {inventory_manifest}"
            )
    ...
    if purpose is DataPurpose.SOFTWARE_TEST and asset.status not in {
        ...
        raise DataActivationError(
            f"software-test binding {role} uses non-test asset {asset_id}"
        )
    return InputBinding(
        role=role,
        asset=asset,
        ...
        available_at_policy=_required_text(
            raw, "available_at_policy", f"input binding {role}"
        ),
        sha256=sha256,
...
def _verify_source_manifest(asset: DataAsset) -> None:
    ...
    if _sha256_file(source_manifest) != raw_hash.lower():
        raise DataActivationError(
            f"source manifest hash mismatch for asset {asset.asset_id}"
        )
...
def _verify_bound_file(binding: InputBinding, path: Path) -> Path:
    ...
    if binding.inventory_manifest is None or binding.inventory_sha256 is None:
        raise DataActivationError(
            f"directory binding {binding.role} has no frozen inventory"
        )
    if _sha256_file(binding.inventory_manifest) != binding.inventory_sha256:
        raise DataActivationError(
            f"inventory hash mismatch for role {binding.role}"
        )
