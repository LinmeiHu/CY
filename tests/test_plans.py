...
def test_cash_dividend_rebases_active_plan_stop_as_append_only_version(
    tmp_path: Path,
) -> None:
    now = datetime(2026, 5, 28, 15, 30, tzinfo=UTC)
    event_time = datetime(2026, 5, 29, 9, 30, tzinfo=UTC)
    ...
    draft = TradingPlan.create(
        symbol="605507.SH",
        family=StrategyFamily.MARKUP_RETEST,
        action=Action.BUY,
        now=now,
        expires_at=now + timedelta(days=3),
        entry_trigger="next tradable open",
    ...
    active = replace(draft, version=2, parent_version=1, status=PlanStatus.ACTIVE)
    repository.append(draft, now)
    repository.append(active, now)

    rebased = repository.rebase_active_for_cash_dividend(
        "605507.SH", 0.4, event_time
    )

    assert len(rebased) == 1
    assert rebased[0].version == 3
    assert rebased[0].protective_stop == pytest.approx(20.6)
    assert len(events.read_all(verify=True)) == 3
