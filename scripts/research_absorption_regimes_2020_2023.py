"""Test the absorption candidate and first-hit barriers by market regime."""
...
OUT = ROOT / "data/audit/chip_absorption_regimes_2020_2023.csv"
...
def main() -> None:
    ...
    COPY (WITH market_daily AS (
      ...
      SELECT d.symbol,d.trade_date,d.close,d.hard_valid,d.buy_blocked_open,
        m.market_close,m.avg20,m.close20,f.average_cost,f.asr,f.concentration_20,
        lag(d.close) OVER w prev_close,lag(f.asr) OVER w prev_asr,
        lag(f.concentration_20) OVER w prev_conc,lead(d.open) OVER w entry_open,
        lead(d.trade_date) OVER w entry_date,lead(d.hard_valid) OVER w entry_valid,
        lead(d.buy_blocked_open) OVER w entry_blocked,lead(d.close,21) OVER w close20_stock
      ...
      SELECT *,CASE WHEN close<=average_cost AND close>prev_close
        ...
        THEN 'ABSORPTION' END signal_type
      FROM x
    ), s AS (
      ...
      FROM s WHERE signal_type IS NOT NULL AND entry_date>trade_date AND entry_open>0
        AND entry_valid AND NOT entry_blocked AND trade_date>=DATE '2020-01-02'
      ...
      SELECT *,CASE WHEN stop_date IS NOT NULL AND (take_date IS NULL OR stop_date<=take_date)
        ...
        THEN -.08 WHEN take_date IS NOT NULL THEN .12 ELSE close20_stock/entry_open-1 END realized_return
      FROM h
    ) SELECT CASE WHEN trade_date<DATE '2023-01-01' THEN 'FIT_2020_2022' ELSE 'HOLDOUT_2023' END sample_group,
      regime,exit_type,count(*) n,avg(realized_return) mean_return,median(realized_return) median_return
      FROM v GROUP BY 1,2,3 ORDER BY 1,2,3
